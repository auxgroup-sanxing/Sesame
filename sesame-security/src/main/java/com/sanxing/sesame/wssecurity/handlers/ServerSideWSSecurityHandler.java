...
public class ServerSideWSSecurityHandler
    extends ExtendedGenericHandler
{
    ...
    public void init( HandlerInfo handlerInfo )
    {
        ...
        String keyStoreResourcePath = (String) config.get( "keyStoreResourcePath" );
        ...
        String ourKeyAlias = (String) config.get( "ourKeyAlias" );
        if ( ourKeyAlias == null )
        {
            throw new RuntimeException( "Missing manadatory parameter: ourKeyAlias" );
        }

        String ourKeyPassword = (String) config.get( "ourKeyPassword" );
        try
        {
            this.ourKey = this.keyStore.getKey( ourKeyAlias, ourKeyPassword.toCharArray() );
            this.ourCertificate = this.keyStore.getCertificate( ourKeyAlias );
        }
        catch ( Exception e )
        {
            this.logger.error( e.getMessage(), e );
    ...
    public boolean handleRequest( MessageContext messageContext )
    {
        ...
        {
            decryptAndOrVerifySignatures( messageContext );
        }
        return true;
    }
    ...
    public void decryptAndOrVerifySignatures( MessageContext messageContext )
    {
        ...
        while ( headerEntriesIter.hasNext() )
        {
            ...
            if ( ( child instanceof SOAPElement ) )
            {
                ...
                else
                {
                    ...
                    if ( "urn:oasis:names:tc:SAML:1.0:assertion".equals( childNamespaceURI ) )
                    {
                        if ( "Assertion".equals( childElement.getLocalName() ) )
                        {
                            processSAMLAssertion( childElement, soapContext, faultActor );
    ...
    private void processSAMLAssertion( Element samlAssertionElement, SOAPMessageContext soapContext, String faultActor )
    {
        this.logger.debug( "Processing SAML Assertion" );
        try
        {
            SAMLAssertion samlAssertion = new SAMLAssertion( samlAssertionElement );
            Iterator samlStatementsIter = samlAssertion.getStatements();
            while ( samlStatementsIter.hasNext() )
            {
                ...
                if ( ( stmt instanceof SAMLAuthenticationStatement ) )
                {
                    ...
                    soapContext.setProperty( "mc_username", authStmt.getSubject().getNameIdentifier().getName() );
                    break;
                }
            }
        }
        ...
        {
            createFaultInContextAndThrow( soapContext, Constants.SOAP_SERVER_FAULT_CODE, "Error processing SAML: ",
                faultActor, e );
        }
    }
