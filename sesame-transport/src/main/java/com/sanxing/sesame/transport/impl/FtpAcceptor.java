...
public class FtpAcceptor extends TaskTransport {
	...
	public void reply(MessageContext context) throws IOException {
		...
		if (resp != null) {
			...
			try {
				ftpClient.setURI(this.uri);
				ftpClient.setProperties(this.properties);
				ftpClient.setBindingProperties(props);
				if (!(ftpClient.connectAddLogin()))
					throw new IOException(
							"can not connect and login to ftp server.");
				if (ftpClient.isConnected()) {
					if (!(ftpClient.uploadFile(context.getAction()
							+ "_response", new ByteArrayInputStream(resp)))) {
			...
			if (ftpClient.isConnected()) {
				...
				for (Object filename : names) {
					if (ftpClient.moveFile((String) filename)) {
						...
						if (in != null) {
							accept(in, (String) filename,
									(String) properties.get("jobName"));

							continue;
						}

						throw new Exception("get file content error!file is:["
								+ filename + "].");
					}
	...
	private void insertBatch(String fileName) throws Exception {
