...
public class FileConnector extends BaseTransport implements Connector {
	...
	public void sendOut(MessageContext context) throws IOException {
		try {
			...
			Map unitProps = (Map) this.unitPropsCache.get(contextPath);
			...
			this.model = ((String) unitProps.get("model"));
			LOG.debug("filename is:" + filename + ",model is:" + this.model);

			if (this.model.equals("get")) {
