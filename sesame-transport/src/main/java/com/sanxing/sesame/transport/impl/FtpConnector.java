...
public class FtpConnector extends BaseTransport implements Connector {
	...
	private void executeCmd(MessageContext context) throws IOException {
		String contextPath = context.getPath();
		Map bindingProps = (Map) this.bindingPropsCache.get(contextPath);
		if (bindingProps == null) {
			bindingProps = new HashMap();
		...
		try {
			...
			if (!(ftpClient.connectAddLogin())) {
				throw new IOException(
						"Can not connect and login to FTP server: " + this.uri);
			}
			...
			if (command.equals("GET")) {
				...
				try {
					if (!(ftpClient.uploadFile(filename, input))) {
						throw new IOException("Put file '" + filename
								+ "' failure!");
					}
					ftpClient.disconnect();
				} finally {
				...
				if (context.getResult() == null) {
					...
					binResult.getOutputStream().write(
							"200OK".getBytes(getCharacterEncoding()));
				}
