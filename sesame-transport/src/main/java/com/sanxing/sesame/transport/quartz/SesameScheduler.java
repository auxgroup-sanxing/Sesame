...
public class SesameScheduler {
	...
	public void removeTrigger(String jobName, String groupName, String timeExp)
			throws SchedulerException {
		this.scheduler.unscheduleJob(jobName + "_" + timeExp, groupName);
	}
	...
	public void modifyTrigger(String jobName, String groupName, String newExp,
			String oldExp) throws SchedulerException, Exception {
		Trigger oldTrigger = this.scheduler.getTrigger(jobName + "_" + oldExp,
				null);
		if (oldTrigger == null)
			throw new Exception("job:[" + jobName
					+ "],can not find the trigger cronExpression:[" + oldExp
					+ "]");
		Trigger trigger = getTrigger(jobName, groupName, newExp);
		trigger.setJobName(jobName);
		this.scheduler
				.rescheduleJob(jobName + "_" + oldExp, groupName, trigger);
	}
	...
	public void registryJob(String jobName, String groupName,
			Class<? extends Job> Job, String timeExp) throws Exception {
		this.scheduler.scheduleJob(new JobDetail(jobName, groupName, Job),
				getTrigger(jobName, groupName, timeExp));
	}

	public void registryJob(Map<String, Object> props) throws Exception {
		...
		this.scheduler.scheduleJob(detail,
				getTrigger(jobName, groupName, cornExp));
	}

	public void registryJob(String jobName, String groupName,
			Class<? extends Job> job, String cornExp,
			Map<String, Object> jobProps) throws Exception {
	...
	public Trigger getTrigger(String jobName, String groupName, String cronExp)
			throws Exception {
		CronTrigger trigger = new CronTrigger(jobName + "_" + cronExp,
				groupName);
	...
	public static void main(String[] args) throws Exception {
		...
		scheduler.scheduleJob(detail,
				impl.getTrigger("job1", "ftp", "0/2 * * * * ?"));

		JobDetail detail2 = new JobDetail("/", "sftp", TaskImpl.class);
		scheduler.scheduleJob(detail2,
				impl.getTrigger("job1", "mail", "0/2 * * * * ?"));
	}

	public void addTrigger(String jobName, String groupName, String timeExp)
			throws Exception {
		if (this.scheduler.getTrigger(jobName + "_" + timeExp, groupName) != null)
			return;
		CronTrigger trigger = new CronTrigger(jobName + "_" + timeExp,
				groupName);
		trigger.setJobName(jobName);
