...
public class TransportRegister implements ShareLibCallback {
	private static Logger LOG = LoggerFactory.getLogger(TransportRegister.class);

	private Map<String, Class<? extends Transport>> registered = new HashMap();

	public void onDispose(File installationDir) {
		Set<String> schemes = this.registered.keySet();
		for (String scheme : schemes) {
			Class clazz = (Class) this.registered.get(scheme);
			LOG.debug("Deregister transport: " + scheme + " -> "
					+ clazz.getName());
				...
				if ((path.isFile()) && (path.getName().endsWith(".jar"))) {
					...
					try {
						...
						while (keys.hasMoreElements()) {
							...
							LOG.debug("Register transport: " + key + " -> "
									+ clazz.getName());
							TransportFactory.register(key, clazz);
							this.registered.put(key, clazz);
						}
					} finally {
						jar.close();
					}
