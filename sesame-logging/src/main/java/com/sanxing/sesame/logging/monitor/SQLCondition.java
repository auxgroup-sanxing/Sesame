...
public class SQLCondition {
	...
	public void add(String column, String operator, Object value, String type,
			boolean allowEmpty) {
		...
		if (type.equals("STR")) {
			if ("like".equalsIgnoreCase(operator)) {
				if (type.equals("DATE")) {
					...
					tmp49_48.condition = tmp49_48.condition + column
							+ " LIKE '" + value + "%' AND ";
				} else {
					SQLCondition tmp92_91 = this;
					tmp92_91.condition = tmp92_91.condition + column
							+ " LIKE '%" + value + "%' AND ";
				}
			} else {
				SQLCondition tmp135_134 = this;
				tmp135_134.condition = tmp135_134.condition + column + " "
						+ operator + " '" + value + "' AND ";
			}
			...
			if ("like".equalsIgnoreCase(operator)) {
				...
				tmp206_205.condition = tmp206_205.condition + column + " = "
						+ " " + value + " AND ";
			} else {
				SQLCondition tmp254_253 = this;
				tmp254_253.condition = tmp254_253.condition + column + " "
						+ operator + " " + value + " AND ";
			}

		} else if (type.equals("DATE")) {
			SQLCondition tmp316_315 = this;
			tmp316_315.condition = tmp316_315.condition + column + " "
					+ operator + "to_timestamp(" + "'" + value + "'"
	...
	public void addDate(String column, String operator, String year,
	...
	public void addTimeStamp(String column, String operator, String day,
			String hour, String minute, String second) {
		if ((isEmpty(day)) || (isEmpty(hour)) || (isEmpty(minute))
				|| (isEmpty(second))) {
			return;
		}
	...
	public String getCondition() {
		if (this.condition.indexOf("WHERE") != -1) {
			return this.condition;
		}

		if (!(isEmpty(this.condition))) {
			if (this.condition.substring(this.condition.length() - 4,
					this.condition.length()).equals("AND ")) {
				this.condition = this.condition.substring(0,
						this.condition.length() - 4);
			}
