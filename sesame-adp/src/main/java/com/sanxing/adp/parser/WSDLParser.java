...
public class WSDLParser {
	...
	private List<PortTypeInfo> portTypeInfos = new LinkedList();
	...
	public void parse4CodeGen(String url) {
		try {
			...
			this.generator.addSchemaHolder(this.schemaHolder);
			if (LOG.isTraceEnabled()) {
				LOG.trace("parse service [" + def.getQName()
						+ " ]ok.......................");
			}
			for (PortTypeInfo typeInfo : this.portTypeInfos) {
				this.generator.addPortType(typeInfo);
			}
	...
	public void parse4runTime(Definition def, ClassLoader loader) {
		try {
			...
			for (PortTypeInfo pi : this.portTypeInfos) {
	...
	public void handleDefination(Definition def) throws Exception {
		if (LOG.isTraceEnabled()) {
			LOG.trace(" begin to analyze wsdl definition");
		}

		this.schemaHolder = new SchemaHolder(def);
		...
		while (iterPortypeNames.hasNext()) {
			...
			for (int i = 0; i < operations.size(); ++i) {
				...
				if (operation.getDocumentationElement() != null) {
					operationInfo.setDescription(operation
							.getDocumentationElement().getTextContent());
					if (LOG.isTraceEnabled()) {
						LOG.trace("found operation commment ["
				...
				if ((operation.getOutput() != null)
					...
					Map outputParts = operation.getOutput().getMessage()
							.getParts();
					Iterator outputPartsIter = outputParts.keySet().iterator();
					while (outputPartsIter.hasNext()) {
						Part part = (Part) outputParts.get(outputPartsIter
								.next());
						PartInfo parameterInfo = new PartInfo();
				...
				while (faultsKeys.hasNext()) {
					...
					while (faultPartsIter.hasNext()) {
						Part faultPart = (Part) faultParts.get(faultPartsIter
								.next());
						PartInfo partInfo = new PartInfo();
						partInfo.setType(2);
						fillParameterInfo(faultPart, partInfo);
						if (LOG.isTraceEnabled()) {
							LOG.trace("found fault [" + count
									+ "] content is [" + partInfo + "]");
						}
	...
	private void fillParameterInfo(Part part, PartInfo parameterInfo) {
		try {
			...
			XmlSchemaElement schemaElement = (XmlSchemaElement) this.schemaHolder
					.getTypes().get(parameterInfo.getElementName());
			if (schemaElement == null) {
				throw new RuntimeException("element ["
						+ parameterInfo.getElementName()
						+ "] not define in schema");
			}
			if ((schemaElement.getSchemaTypeName() != null)
					&& (schemaElement.getSchemaTypeName().getNamespaceURI()
			...
			parameterInfo.setJavaType(XJUtil.ns2package(namespace) + "."
					+ StringUtils.capitalize(localPart));
		} catch (Exception e) {
