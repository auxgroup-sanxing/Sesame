...
public class CodeGenerator {
	...
	private List<PortTypeInfo> interfaces = new ArrayList();
	private String targetPackageName;
	private String targetFolder;

	public CodeGenerator() {
		this.p = new Properties();
		this.p.setProperty("resource.loader", "class");
		this.p.setProperty("class.resource.loader.class",
				"org.apache.velocity.runtime.resource.loader.ClasspathResourceLoader");
		this.p.setProperty("input.encoding", "UTF-8");
		try {
			Velocity.init(this.p);
		} catch (Exception e) {
	...
	public void generate() throws Exception {
		for (PortTypeInfo portType : this.interfaces) {
			LOG.trace("generate code for portype " + portType);
			generateInterface(portType);
		}
		generateJAXBBean(this.targetPackageName);
	}

	private void generateInterface(PortTypeInfo portTypeInfo) throws Exception {
		if ((this.targetPackageName == null)
				|| (this.targetPackageName.trim().equals(""))) {
			this.targetPackageName = XJUtil.ns2package(portTypeInfo.getName()
					.getNamespaceURI());
		}
		generateExceptionClass(portTypeInfo);
		generateByTemplate(portTypeInfo,
				"com/sanxing/adp/compiler/interface.vm", "");
	}

	public void generateByTemplate(PortTypeInfo portTypeInfo,
			String templateFile, String classNameSuffix) {
		String clazzName = StringUtils.capitalize(portTypeInfo.getName()
				.getLocalPart()) + classNameSuffix;
		...
		try {
			...
			context.put("namcepace", portTypeInfo.getDefinationName()
					.getNamespaceURI());
			context.put("portTypeName", portTypeInfo.getDefinationName()
					.getLocalPart());
			context.put("operations", portTypeInfo.getOperations());
			context.put("date", date);
			context.put("className", clazzName);
			context.put("packageName", this.targetPackageName);

			FileWriter fw = getFileWriter(clazzName, this.targetPackageName);
	...
	private void generateJAXBBean(String packageName) throws Exception {
		JAXBCompiler compiler = new JAXBCompiler(this.schemaHolder);
		compiler.jaxbCompile(this.targetFolder, packageName);
	}
		...
		for (OperationInfo operation : portTypeInfo.getOperations())
			for (FaultInfo fault : operation.getFaults()) {
				Template template = Velocity.getTemplate(
						"com/sanxing/adp/compiler/fault.vm", "UTF-8");
				VelocityContext context = new VelocityContext();
	...
	private FileWriter getFileWriter(String clazzName, String packageName)
		...
		FileWriter fw = new FileWriter(ifaceFilePath + File.separator
				+ clazzName + ".java");
		return fw;
	}
}