...
class NameUtil {
	...
	protected String toMixedCaseName(List<String> ss, boolean startUpper) {
		...
		if (!(ss.isEmpty())) {
			sb.append((startUpper) ? (String) ss.get(0)
					: toLowerCase((String) ss.get(0)));
	...
	protected String toMixedCaseVariableName(String[] ss, boolean startUpper,
			boolean cdrUpper) {
		if (cdrUpper)
	...
	public String toConstantName(List<String> ss) {
		StringBuilder sb = new StringBuilder();
		if (!(ss.isEmpty())) {
			sb.append(toUpperCase((String) ss.get(0)));
			for (int i = 1; i < ss.size(); ++i) {
				sb.append('_');
				sb.append(toUpperCase((String) ss.get(i)));
			}
		}
		return sb.toString();
	}
