...
public class ReturnMethodProcessor extends BaseMethodProcessor {
	...
	private Element invokeSingleResultTx(OperationInfo oper, Object tx,
			Object[] paramObjets) throws AppException {
		Element root = null;
		...
		if (XJUtil.isPrimitive(oper.getResult().getJavaType())) {
			String elementNS = oper.getResult().getElementName()
					.getNamespaceURI();
			String elementName = oper.getResult().getElementName()
					.getLocalPart();
