	...
	public static void main(String[] args) {
		try {
			...
			if (args.length == 1) {
				if (args[0].equals("-i")) {
					System.out
							.println("please input wsdl file absolute path (incluede file name)");
					...
					System.out
							.println("please input target dir ,default is current dir");
					...
					parser.parse4CodeGen(new File(wsdlFileLocation).toURL()
							.toString());
				} else {
					pringUsage();
				}
