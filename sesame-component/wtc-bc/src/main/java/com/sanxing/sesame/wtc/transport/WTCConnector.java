...
public class WTCConnector extends BaseTransport
  implements Connector
{
  ...
  private byte[] reform(SOPRequest request, BinarySource source)
  {
    ...
    String key = "offset";
    byte[] offset = (byte[])source.getProperty(key);
    if (offset == null) {
      offset = new byte[20];
    }
    ...
    String channelFrom = (String)source
      .getProperty("channel_from");
    String channelTo = (String)source.getProperty("channel_to");
    if (LOG.isDebugEnabled()) {
      LOG.debug("channelFrom: " + channelFrom + "; channelTo: " + 
        channelTo);
    }
    ...
    throws BindingException
  {
    ...
    try
    {
      ...
      TuxedoConnectionFactory tuxedoFactory = (TuxedoConnectionFactory)ctx
    ...
    throws Exception
  {
    ...
    System.arraycopy(recvBytes, 141, 
      errBytes, 0, 
      7);
    ...
    if (errcode.equalsIgnoreCase("AAAAAAA"))
    {
      ...
      LOG.debug("WTC:Transaction fail, the fault name: fault" + 
        tranCode);
  ...
  public void sendOut(MessageContext context) throws BindingException, IOException
  {
