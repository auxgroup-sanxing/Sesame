...
public class WTCAcceptor extends BaseTransport
  implements Acceptor, WTCHandler
{
  ...
  protected BinaryResult doSend(String contextPath, BinarySource source) throws MessagingException, BindingException
  {
  ...
  private byte[] response(SOPRequest req, BinaryResult result) {
    ...
    byte[] offsetBytes = (byte[])result.getProperty("offset");
    if (offsetBytes == null) {
  ...
  private BinaryResult sendRequest(SOPRequest req) throws MessagingException, BindingException
  {
