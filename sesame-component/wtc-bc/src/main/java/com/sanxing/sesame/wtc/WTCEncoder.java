...
public class WTCEncoder extends EncodeFSV
{
    ...
    try {
      ...
      if (getProperty(key) != null) {
        ...
        if (prop.getChannelFrom() != null) {
          result.setProperty("channel_from", 
            prop.getChannelFrom());
        }
        if (prop.getChannelTo() != null) {
          result.setProperty("channel_to", 
            prop.getChannelTo());
        }
  ...
  public void encodeVFiled(OutputStream output, ValidateVFormat vVFormat, String elementType, String encodeCharset, String elementValue, String elementName)
    throws FormatException
  {
  ...
  private void encodeVFiled(OutputStream output, VariableElement element) throws FormatException
  {
    try {
  ...
  private void encodeSingleVFiled(OutputStream output, VariableElement element) throws Exception
  {
    ...
    if (skipElement(element.getElementName())) {
      encodeSkipField(key, element.getElementName(), 
        element.getElementValue());
      return;
    }

    encodeHead(output, headBytes, headLength, headBlank, headAlign, 
      headRadix, element.getEncodeCharset(), element.getElementName());

    if ("string".equals(element.getElementType()))
      output.write(element.getElementValue().getBytes(
        element.getEncodeCharset()));
    else
      output.write(HexBinary.decode(element.getElementValue()));
  }

  public void encodeHead(OutputStream output, byte[] headBytes, int headLength, char headBlank, String headAlign, int headRadix, String encodeCharset, String elementName)
    throws FormatException
  {
    byte[] bytes = fillBlankByte(headBytes, headLength, headBlank, 
  ...
  public static byte[] fillBlankByte(byte[] elementBytes, int length, char blank, String align, String encodeCharset, String elementName)
    throws FormatException
  {
    ...
    if (len > length)
      throw new FormatException(
        "element:[" + 
        elementName + 
        "],length of elementValue is more longer then xsd defind len:[" + 
        length + "]");
    if (len == length) {
      return elementBytes;
    }
  ...
  public void encodeNonNumber(OutputStream output, String elementValue, int length, char blank, String align, String encodeCharset, String elementName)
    throws FormatException
  {
    ...
    try {
      ...
      if ((eleLen > length) && (elementName.equalsIgnoreCase("errorCode"))) {
        ...
        if (filtered.length() > 7)
          elementValue = filtered.substring(0, 7);
        else {
          elementValue = filtered;
        }
      }
      chgValue = RidFillBlank.fillBlank(elementValue, length, blank, 
  ...
  public boolean skipElement(String elementName) {
    if ((elementName.equalsIgnoreCase("channel_from")) || 
      ...
      (elementName.equalsIgnoreCase("pin_flag")) || 
      (elementName.equalsIgnoreCase("pin_seed"))) {
      return true;
    }
    return false;
  }
  ...
  class SOProperty
  {
    ...
    {
      return "SOProperty [offset=" + Arrays.toString(this.offset) + 
        ", channelTo=" + this.channelTo + ", channelFrom=" + 
        this.channelFrom + ", pinSeed=" + this.pinSeed + ", pinFlag=" + 
        this.pinFlag + "]";
    }
  }
}
