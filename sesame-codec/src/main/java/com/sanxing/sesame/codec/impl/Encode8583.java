...
public class Encode8583
    implements Encoder
{
    public void encode( org.jdom.Element xmlElement, XmlSchemaElement xsdElement, OutputStream output,
    ...
    private void encodeMessage( XmlSchemaType xsdType, org.jdom.Element xmlElement, OutputStream output,
                                String encodeCharset )
        throws FormatException
    {
        try
        {
            ...
            while ( iterator.hasNext() )
            {
                ...
                else if ( "V".equals( kind ) )
                {
                    ...
                    encodeVValue( output, encodeCharset, elementType, elementValue, elementName, vVFormat );
                }
            }
        }
    ...
    private void encodeBitSet( org.jdom.Element xmlElement, Iterator tempIter, BitSet bs )
        throws FormatException
    {
    ...
    protected void encodeVValue( OutputStream output, String encodeCharset, String elementType, String elementValue,
                                 String elementName, ValidateVFormat8583 vVFormat )
        throws FormatException
    {
        ...
        try
        {
            ...
            {
                headValue = Integer.toString( elementValue.getBytes().length, headRadix );
                if ( headRadix == 2 )
                {
            ...
            {
                throw new FormatException( "element:[" + elementName + "],kind is [V],type is:[" + elementType
                    + "]error!" );
            }
            String realHeadValue = null;
            realHeadValue =
                RidFillBlank.fillBlank( headValue, headLen, headBlank, headAlign, encodeCharset, elementName );
            ...
            if ( "string".equals( elementType ) )
            {
                if ( 1 == compress )
                {
                    output.write( BCD.str2bcd( elementValue, headAlign, headBlank ) );
                    return;
                }
    ...
    protected void encodeFValue( OutputStream output, String encodeCharset, String elementType, String elementValue,
                                 String elementName, ValidateFFormat8583 vFFormat )
        throws FormatException
    {
        try
        {
            ...
            throw new FormatException( "element:[" + elementName + "],attribute kind is:[F],attribute type is:["
                + elementType + "]error!" );
        }
        catch ( FormatException fe )
        {
        ...
        throws FormatException
    {
        ...
        try
        {
            ...
            if ( message == null )
            {
                throw new FormatException( "can not get doc from parameter [DOMSource]" );
            }
            XmlSchemaElement schemaEl = schema.getElementByName( elementName );
            encode( message.getRootElement(), schemaEl, output, charset );
        }
