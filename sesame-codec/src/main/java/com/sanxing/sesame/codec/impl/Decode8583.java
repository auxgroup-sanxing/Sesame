...
public class Decode8583 implements Decoder {
	public org.jdom2.Element decode(byte[] message, int length, String charset,
		...
		if (length > message.length)
			throw new FormatException(
					"Decode8583 ,parameter length value bigger then real buffer length error!");
		...
		try {
			if (schemaElement == null)
				throw new FormatException(
						"Decode8583,parameter  [XmlSchemaElement] is null!");
			...
			if (msgBuf.position() != msgBuf.limit())
				throw new FormatException(
						"parameter [length] bigger then real need buffer length");
	...
	protected void decodeMessage(ByteBuffer recvBuf, XmlSchemaType schemaType,
		...
		try {
			...
			while (elements.hasNext()) {
				...
				org.w3c.dom.Element format = CodecUtil.getXSDFromat(element,
						elementName);
				...
				if (!(bs.get(id - 1))) {
					...
					if ("F".equals(kind)) {
						...
						if ("string".equals(elementType)) {
							...
							String eleValue = getFField(recvBuf, len, blank,
									compress, align, charset, elementType);
							...
							byte[] temp = new byte[len];
							recvBuf.get(temp);
							childOfElementMessage.addContent(HexBinary
									.encode(temp));
						}
						...
						String eleValue = getVField(recvBuf, headLen,
								headBlank, compress, headAlign, headCompress,
								headRadix, elementType, charset, elementName);
						childOfElementMessage.addContent(eleValue);
					} else {
						throw new FormatException(
								"element format,attribute kind value is:["
										+ kind + "]error");
					}
					root.addContent(childOfElementMessage);
				}
			...
			if (!(tempBs.equals(bs)))
				throw new FormatException(
						"bit map error! bitBuffer is not match with schema!");
		} catch (FormatException fe) {
			throw fe;
		} catch (Exception e) {
			throw new FormatException(e.getMessage(), e);
	...
	public String getFField(ByteBuffer buf, int len, int blank, int compress,
			String align, String charset, String elementType)
		...
		try {
			...
			if ("L".equals(align)) {
				result = RidFillBlank.ridRightBlank(temp, blank, elementType,
						charset);
			} else {
				result = RidFillBlank.ridLeftBlank(temp, blank, elementType,
						charset);
			}
	...
	public String getVField(ByteBuffer buf, int headLen, int headBlank,
			int compress, String headAlign, int headCompress, int headRadix,
			String elementType, String charset, String elementName)
			throws FormatException {
		String result = null;
		try {
			String realHeadValue = getHeadValue(buf, headLen, headBlank,
					headCompress, headAlign, headRadix, charset);
			...
			result = getEleValue(buf, compress, headAlign, elementType,
					charset, result, eleLen, elementName);
		} catch (Exception e) {
			throw new FormatException(e.getMessage(), e);
		}
	...
	private String getHeadValue(ByteBuffer buf, int headLen, int headBlank,
			int headCompress, String headAlign, int headRadix, String charset)
			throws FormatException {
		String result = null;
		byte[] temp = (byte[]) null;
		try {
			...
			if ("L".equals(headAlign)) {
				result = RidFillBlank.ridRightBlank(temp, headBlank,
						elementType, charset);
			} else {
				result = RidFillBlank.ridLeftBlank(temp, headBlank,
						elementType, charset);
			}
		} catch (UnsupportedEncodingException e) {
			throw new FormatException(e.getMessage(), e);
	...
	private String getEleValue(ByteBuffer buf, int compress, String align,
			...
			String elementName) throws UnsupportedEncodingException,
			FormatException {
		if ("string".equals(elementType)) {
