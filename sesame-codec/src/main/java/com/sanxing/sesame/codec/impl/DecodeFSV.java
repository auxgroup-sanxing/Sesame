    ...
    implements Decoder
{
        ...
        throws FormatException
    {
        ...
        {
            throw new FormatException( "decode,parameter length:[" + length + "] bigger then real buffer length:["
        ...
        try
        {
            ...
            {
                throw new FormatException( "decode,parameter [length:" + msgBuf.limit()
                    + "] bigger then the real need [length:" + msgBuf.position() + "]" );
            }
        }
    ...
    protected void decodeMessage( ByteBuffer message, Iterator<?> elements, String charset,
                                  org.jdom.Element elementMessage, XmlSchema schema )
        throws FormatException
    {
        try
        {
            while ( elements.hasNext() )
            {
                ...
                if ( xsType instanceof XmlSchemaSimpleType )
                {
                    ...
                    switch ( partTypeMap.get( kind ).intValue() )
                    {
                        case 0:
                                ...
                                || ( "hexBinary".equals( type ) ) )
                            {
                                ...
                                decodeNonNumber( message, charset, childOfElementMessage, type, len, blank, align );
                            }
                            else
                            {
                                ...
                                decodeNumber( message, childOfElementMessage, endian, type );
                            }
                        ...
                        case 1:
                            ...
                            {
                                decodeSeparator( message, childOfElementMessage, format, charset, elementName );
                                elementMessage.addContent( childOfElementMessage );
                            }
                            else
                            {
                                throw new FormatException( "element:[" + element.getName() + "],type is:[" + type
                                    + "] error! unsupported type!" );
                            }
                            break;
                        case 2:
                            ValidateVFormat vVFormat = new ValidateVFormat( element, format );
                            if ( ( "string".equals( type ) ) || ( "hexBinary".equals( type ) ) )
                            {
                                decodeVField( message, charset, elementMessage, element.getName(),
                                    childOfElementMessage, type, vVFormat );
                            }
                            else
                            {
                                throw new FormatException( "element:[" + element.getName() + "],type is:[" + type
                ...
                if ( ( xsType instanceof XmlSchemaComplexType ) && ( element.getSchemaType().getName() != null ) )
                {
                    elementMessage.addContent( childOfElementMessage );
                    decodeMessage( message, CodecUtil.getElements( element.getSchemaType() ), charset,
                        childOfElementMessage, schema );
                }
                else
                {
                    ...
                    try
                    {
                        ...
                        if ( occurs != null )
                        {
                            ...
                            org.jdom.Element repeat_num =
                                (org.jdom.Element) XPath.selectSingleNode( elementMessage, path );

                            if ( repeat_num == null )
                            {
                                throw new FormatException( "element:[" + element.getName()
                                    + "],can not find the element by path[" + path + "]!" );
                            }
                            loopNum =
                                Integer.parseInt( ( repeat_num.getText().equals( "" ) ) ? "0" : repeat_num.getText() );
                            if ( loopNum < 0 )
                            {
                                throw new FormatException( "the repeat element [" + repeat_num.getName()
                                    + "]'s value is invalid" );
                            }
                            if ( loopNum == 0 )
                            {
                                org.jdom.Element detail = new org.jdom.Element( elementName );
                        ...
                        for ( int i = 0; i < loopNum; ++i )
                        {
                            org.jdom.Element detail = new org.jdom.Element( elementName );
                            elementMessage.addContent( detail );
                            decodeMessage( message, CodecUtil.getElements( loopComplexType ), charset, detail, schema );
                        }
                    }
    ...
    public void decodeVField( ByteBuffer message, String charset, org.jdom.Element elementMessage, String elementName,
                              org.jdom.Element childOfElementMessage, String type, ValidateVFormat vVFormat )
        throws FormatException
    {
        ...
        decodeHead( message, charset, tempElement, headRadix, headLength, headBlank, headAlign );
        ...
        {
            throw new FormatException( " element:[" + elementName + "],can not get the message length" );
        }
    }
    ...
    private ByteBuffer getUnsignedNumberBuf( ByteBuffer message, String endian, int len )
    {
    ...
    private void decodeSeparator( ByteBuffer message, org.jdom.Element childOfElementMessage,
                                  org.w3c.dom.Element format, String charset, String elementName )
        throws FormatException
    {
        boolean flage = true;
        try
        {
            ...
            if ( ( hexLimit != null ) && ( !( "".equals( hexLimit ) ) ) )
            {
                ...
                {
                    throw new FormatException( "element:[" + elementName + "],can not find begin limit error!" );
                }
                if ( ( hexLimitLength == separatorLength ) && ( hexLimit.equals( hexSeparator ) ) )
                {
                    throw new FormatException( "element:[" + elementName + "],limit can not equals separator" );
                }

                for ( int pos = message.position(); message.position() < message.limit(); message.position( ++pos ) )
                {
                    ...
                    if ( ( !( hexSeparator.equals( new String( temp, charset ) ) ) )
                        || ( message.position() - oldPos - hexLimitLength - separatorLength < hexLimitLength ) )
                    {
                        continue;
                    }
                    byte[] limitTempend = new byte[hexLimitLength];
                    message.position( message.position() - hexLimitLength - separatorLength );
                    message.get( limitTempend );

                    if ( hexLimit.equals( new String( limitTempend, charset ) ) )
                    {
                        int messageLen = message.position() - oldPos - ( 2 * hexLimitLength );
                        byte[] messageBuf = new byte[messageLen];
                        message.position( oldPos + hexLimitLength );
                        message.get( messageBuf );
                        childOfElementMessage.addContent( new String( messageBuf, charset ) );
                        message.position( message.position() + hexLimitLength + separatorLength );
                        flage = false;
                        break;
                    }
            ...
            if ( flage )
            {
                ...
                if ( ( hexLimit != null ) && ( !( "".equals( hexLimit ) ) ) )
                {
                    ...
                    if ( !( hexLimit.equals( new String( limitBuf, charset ) ) ) )
                    {
                        throw new FormatException( "element:[" + elementName + "],can not find end limit error!" );
                    }
                    byte[] remainBuf = new byte[message.position() - oldPos - ( 2 * hexLimitLength )];
                    message.position( oldPos + hexLimitLength );
                    message.get( remainBuf, 0, remainBuf.length );
                    childOfElementMessage.addContent( new String( remainBuf, charset ) );
                    message.position( message.position() + hexLimitLength );
                }
                else
                {
    ...
    private void decodeNumber( ByteBuffer message, org.jdom.Element childOfElementMessage, String endian, String type )
        throws FormatException
    {
        if ( type.equals( "int" ) )
        {
        ...
        {
            throw new FormatException( "type is:[" + type + "] error! unsupported type!" );
        }
    }

    public void decodeHead( ByteBuffer message, String charset, org.jdom.Element childOfElementMessage, int headRadix,
        ...
        throws FormatException
    {
        ...
        {
            childOfElementMessage.addContent( RidFillBlank.ridRightBlank( temp, headBlank, type, charset ) );
        }
        else
        {
            childOfElementMessage.addContent( RidFillBlank.ridLeftBlank( temp, headBlank, type, charset ) );
        }
    }

    private void decodeNonNumber( ByteBuffer message, String charset, org.jdom.Element childOfElementMessage,
                                  String type, int length, int blank, String align )
        throws FormatException
    {
        byte[] temp = new byte[length];
        message.get( temp );
        if ( "L".equals( align ) )
        {
            childOfElementMessage.addContent( RidFillBlank.ridRightBlank( temp, blank, type, charset ) );
        }
        else
        {
            childOfElementMessage.addContent( RidFillBlank.ridLeftBlank( temp, blank, type, charset ) );
        }
    }
        ...
        throws FormatException
    {
        ...
        if ( schema == null )
        {
            throw new FormatException( "In decodeFSV,can not find schema in BinarySource!" );
        }
        ...
        try
        {
            ...
            stream.read( bytes );
            org.jdom.Element element = decode( bytes, length, charset, schemaEl, schema );
            element.detach();
        ...
        throws FormatException
    {
        if ( schema == null )
        {
            throw new FormatException( "In decodeFSV,the parameter [XmlSchema] is null!" );
        }
