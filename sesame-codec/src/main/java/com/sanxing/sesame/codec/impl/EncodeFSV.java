...
public class EncodeFSV implements Encoder {
		...
		try {
			...
			if (message == null)
				throw new FormatException(
						"can not get document from parameter [XMLSource]!");
	...
	public void encode(org.jdom.Element xmlElement, XmlSchema schemaElement,
		...
		if (schemaElement.getElementByName(xmlElement.getName()) == null)
			throw new FormatException("can not get schema element by name:["
					+ xmlElement.getName() + "]!");
		XmlSchemaType xsdType = schemaElement.getElementByName(
				xmlElement.getName()).getSchemaType();
		...
		encodeMessage(CodecUtil.getElements(xsdType), xmlElement, output,
				encodeCharset, schemaElement);
	}
			...
			while (elements.hasNext()) {
					...
					if (schema.getElementByName(element.getRefName()) == null)
						throw new FormatException(
								"can not get element from schema by name:["
										+ element.getRefName() + "]!");
					type = schema.getElementByName(element.getRefName())
							.getSchemaType();
				} else {
				...
				if (type instanceof XmlSchemaSimpleType) {
					...
					if (elementValue == null) {
						...
						throw new FormatException(
								"in xmlDoc, can not find the element: ["
										+ elementName + "]");
					}

					String kind = format.getAttribute("kind");
					ValidateField.validateKind(elementName, kind);
					kindInt = ((Integer) this.partTypeMap.get(kind)).intValue();
					...
					case 0:
						...
						if (("string".equals(elementType))
							...
							if (elementType.equals("hexBinary"))
								encodeHexBinary(output, elementValue, length,
										blank, align, elementName);
							else
								encodeNonNumber(output, elementValue, length,
										blank, align, encodeCharset,
										elementName);
						} else {
							if (!(this.typeList.contains(elementType))) {
								throw new FormatException("not support type ["
										+ elementType + "]");
							}

							String endian = vFFormat.getEndian();
							encodeNumber(output, elementValue, endian,
									elementType);
						}
						break;
					case 1:
						...
						if ((limit != null) && (!("".equals(limit))))
							try {
								...
								output.write(elementValue
										.getBytes(encodeCharset));
								output.write(limit.getBytes(encodeCharset));
					...
					case 2:
						...
						if (("string".equals(elementType))
								|| ("hexBinary".equals(elementType)))
							encodeVFiled(output, vVFormat, elementType,
									encodeCharset, elementValue, elementName);
						else
							throw new FormatException("element:["
									+ element.getName() + "],type is:["
									+ elementType
				...
				if ((type instanceof XmlSchemaComplexType)
					...
					encodeMessage(
							...
							xmlElementChild, output, encodeCharset, schema);
				} else {
					if ((!(type instanceof XmlSchemaComplexType))
							|| (element.getSchemaType().getName() != null))
						continue;
					...
					if (element.getAnnotation() != null) {
						...
						for (Iterator it = annColl.getIterator(); it.hasNext();) {
							...
							if (o instanceof XmlSchemaAppInfo) {
								...
								occurs = CodecUtil.getChildEleOfAppinfo(list,
										"occurs");
							}
						}
					}
					...
					if (occurs != null) {
						try {
							String path = occurs.getAttributes()
							...
							if (xmlChild == null)
								throw new FormatException(
										"in xmlDoc,can not find the child element:["
												+ elementName
							...
							if (repeat_num == null) {
								throw new FormatException(
										"in xmlDoc,can not find the repeat_num element by path:["
												+ path + "]!");
							}
					...
					for (int i = 0; i < loopNum; ++i) {
						...
						if (xmlElementChild == null)
							throw new FormatException(
									"in xmlDoc,can not find the " + (i + 1)
											+ " childElement:[" + elementName
											+ "] of element:[" + xmlElement
											+ "]!");
						encodeMessage(CodecUtil.getElements(loopComplexType),
								xmlElementChild, output, encodeCharset, schema);
					}
				}
			}
	...
	private void encodeHexBinary(OutputStream output, String elementValue,
			int length, int blank, String align, String elementName)
			throws FormatException {
		try {
	...
	private void encodeNumber(OutputStream output, String elementValue,
			String endian, String elementType) throws Exception {
	...
	public void encodeNonNumber(OutputStream output, String elementValue,
			int length, char blank, String align, String encodeCharset,
			String elementName) throws FormatException {
		String chgValue = elementValue;
		chgValue = RidFillBlank.fillBlank(elementValue, length, blank, align,
				encodeCharset, elementName);
		try {
	...
	public void encodeVFiled(OutputStream output, ValidateVFormat vVFormat,
			String elementType, String encodeCharset, String elementValue,
			String elementName) throws FormatException {
		...
		try {
			...
			if ("string".equals(elementType)) {
				headValue = Integer.toString(
						elementValue.getBytes(encodeCharset).length, headRadix);
			...
			encodeHead(output, headValue, headLength, headBlank, headAlign,
					headRadix, encodeCharset, elementName);
	...
	public void encodeHead(OutputStream output, String headValue,
			int headLength, char headBlank, String headAlign, int headRadix,
			String encodeCharset, String elementName) throws FormatException {
		String chgValue = headValue;
		chgValue = RidFillBlank.fillBlank(headValue, headLength, headBlank,
				headAlign, encodeCharset, elementName);
		try {
			output.write(chgValue.getBytes(encodeCharset));
		} catch (IOException e) {
			throw new FormatException(e.getMessage(), e);
