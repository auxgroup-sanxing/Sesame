...
public class ValidateField {
	public static String validateAlign(String elementName, String align)
		...
		if ((!("L".equals(align))) && (!("R".equals(align))))
			throw new FormatException(
					"element:["
							+ elementName
							+ "],element format attribute [align] or child element head attribute [align], value is:["
							+ align + "]error!");
		return align;
	}
	...
	public static int validateEId(String idStr, String elementName)
		...
		if ((id < 2) || (id > 128))
			throw new FormatException("in xsdDoc,element:[" + elementName
					+ "] , format,attribute [id],value:[" + id + "]error!");
		return id;
	}
	...
	public static void validate8583Kind(String elementName, String kind)
		...
		if ((!("F".equals(kind))) && (!("V".equals(kind))))
			throw new FormatException("in xsdDoc,element:[" + elementName
					+ "],element format,the attribute [kind] value is:[" + kind
					+ "] error!");
	}

	public static char validateBlank(String elementName, char blank,
			String blankStr) throws FormatException {
		if ((!("".equals(blankStr))) && (blankStr != null)) {
			if (blankStr.getBytes().length > 1) {
				...
				throw new FormatException(
						"in xsdDoc,element:["
								+ elementName
								+ "],element format attribute [blank] or child element head attribute [blank], value:["
								+ blankStr
	...
	public static String validateEndian(String elementName, String endian)
		...
		if ((!("big".equals(endian))) && (!("little".equals(endian))))
			throw new FormatException("element:[" + elementName + "],endian:["
					+ endian + "] error!");
		return endian;
	}
		...
		if (("".equals(kind)) || (kind == null))
			throw new FormatException(
					"in xsdDoc,element:["
							+ elementName
							+ "], element format,the attribute [kind] do not define or it have no value!");
		if ((!("F".equals(kind))) && (!("S".equals(kind)))
				&& (!("V".equals(kind))))
			throw new FormatException("in xsdDoc,element:[" + elementName
					+ "],element format,the attribute [kind] value is:[" + kind
					+ "] error!");
