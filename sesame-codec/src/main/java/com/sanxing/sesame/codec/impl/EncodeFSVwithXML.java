...
public class EncodeFSVwithXML
    extends EncodeFSV
{
    private static XMLOutputter outputter = new XMLOutputter();

    @Override
    public void encode( Element xmlElement, XmlSchema schema, OutputStream output, String encodeCharset )
        throws FormatException
    {
        if ( xmlElement == null )
        {
            throw new FormatException( "function messageEncode,the parameter [xmlElement] is null" );
        }
        XmlSchemaType xsdType = schema.getElementByName( xmlElement.getName() ).getSchemaType();
        ...
        encodeMessage( fsvElements.iterator(), xmlElement, output, encodeCharset, schema );
