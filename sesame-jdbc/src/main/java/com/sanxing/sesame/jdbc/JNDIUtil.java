		...
		if (onSesame) {
			...
			try {
				...
				try {
					...
					props.load(JNDIUtil.class.getClassLoader()
							.getResourceAsStream("db.config"));
					String uniqueName = "sesame.adp.test";
					int maxPoolSize = GetterUtil.getInteger(
							props.getProperty("max-pool-size"), 10);
					String jndiName = GetterUtil.getString(props
