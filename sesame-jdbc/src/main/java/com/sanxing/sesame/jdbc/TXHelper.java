...
public class TXHelper
{
    ...
    private static TransactionManager getFromWebLogicFactory()
        throws Exception
    {
        if ( wlsTM == null )
        {
            try
            {
                Class clazz = Class.forName( "weblogic.transaction.TransactionHelper" );
                Method create = clazz.getMethod( "getTransactionHelper", new Class[0] );
                Object obj = create.invoke( clazz, null );

                Method m = clazz.getMethod( "getTransactionManager", new Class[0] );
    ...
    private static TransactionManager getTransactionManager()
    {
        ...
        try
        {
            ...
            {
                tm = (TransactionManager) JNDIUtil.getInitialContext().lookup( "java:comp/UserTransaction" );
            }
        }
        catch ( Exception e )
        {
    ...
    {
        TransactionManager tm = getTransactionManager();
        TXRecord rec = getTXRecords().getLast();
    ...
    {
        TransactionManager tm = getTransactionManager();
        TXRecord rec = getTXRecords().removeLast();
        if ( rec.getType() == 2 )
        {
