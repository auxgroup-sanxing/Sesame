...
public class Reverter {
	...
	private Log reverseLog = LogFactory.getLog("sesame.reverse");

	public void pushReverse(Reverse reverse) {
		this.reverses.add(reverse);
	}

	public void execute(String group, CalloutException e) {
		Collections.sort(this.reverses);

		String exceptionKey = e.getKey();
		String exceptionMsg = e.getMessage();
		for (Reverse reverse : this.reverses) {
			...
			if (clause != null)
				if (clause.isInstantly()) {
					try {
						...
						dataCtx.getExecutionContext().put("process.faultcode",
								e.getKey());
						dataCtx.getExecutionContext().put(
								"process.faultstring", e.getMessage());

						ActionUtil.bachInvoke(dataCtx, clause.getActions()
								.iterator());
					} catch (AbortException localAbortException) {
					} catch (Throwable t) {
						LOG.error(
								"Reverse failure, write log for reverse task",
								t);

						writeLog(reverse.getSnapshot(), clause.getActions()
								.iterator());
					}
				} else {
	...
	public void writeLog(DataContext dataCtx, Iterator<?> actions) {
		...
		try {
			NamespaceContext namespaceCtx = (NamespaceContext) dataCtx
					.getExecutionContext().get("process.namespaces");
			...
			while (actions.hasNext()) {
				...
				if (actionEl.getName().equals("callout")) {
					...
					String strServiceName = addressEl
							.getChildTextTrim("service-name");
					QName serviceName = ((strServiceName == null) || (strServiceName
							.length() == 0)) ? null : QName
							.valueOf(strServiceName);
					String strInterfaceName = addressEl
							.getChildTextTrim("interface-name");
					QName interfaceName = (strInterfaceName == null) ? null
							: QName.valueOf(strInterfaceName);
					String strOperationName = addressEl
							.getChildTextTrim("operation-name");
					QName operationName = (strOperationName == null) ? null
							: QName.valueOf(strOperationName);
					String endpointName = addressEl
							.getChildTextTrim("endpoint-name");

					MessageExchange me = engine.getExchangeFactory()
					...
					me.setProperty("sesame.exchange.consumer", engine
					...
					me.setService(new QName(
							"http://www.sanxing.com/sesame/revert",
							"RevertService"));
					...
					this.reverseLog.debug("Write reverse log success");
				} else {
			...
			this.reverseLog.error("Write Reverse log failure");
