...
public class TryCatchAction extends AbstractAction implements Constant {
	...
	public void dowork(DataContext dataCtx) throws AppException {
		Iterator tryActions = this.actionEl.getChild("try").getChildren()
				.iterator();
		try {
			...
			if (e instanceof KeyedErr) {
				...
				if ((e instanceof ActionException)
						&& (e.getCause() instanceof KeyedErr)) {
			...
			List catches = this.actionEl.getChildren("catch");
			for (Iterator iter = catches.iterator(); iter.hasNext();) {
				...
				for (int i = 0; i < exceptionKeys.length; ++i) {
					...
					if (key.startsWith("$")) {
						exceptionKeys[i] = dataCtx
								.getVariable(key.substring(1)).toString();
					}
				}
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Catchable exceptions: [" + strKeys + "]");
				}
				boolean catchable = Catcher.isCatchable(exceptionKey,
						exceptionKeys);
				if (catchable) {
					catched = true;
					if (LOGGER.isDebugEnabled()) {
					...
					dataCtx.getExecutionContext().put("process.faultcode",
							exceptionKey);
					dataCtx.getExecutionContext().put("process.faultstring",
							exceptionMsg);

					ActionUtil.bachInvoke(dataCtx, catchEl.getChildren()
							.iterator());

					String rethrow = catchEl
							.getAttributeValue("throw", "false");
