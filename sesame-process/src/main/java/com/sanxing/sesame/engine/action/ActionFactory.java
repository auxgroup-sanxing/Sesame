...
public class ActionFactory
{
    ...
    public static Action getInstance( Element actionEl )
    {
        try
        {
            Action action = actionInstances.get( actionEl );
            if ( action == null )
            {
                String actionName = actionEl.getName();
                Class clazz = actionNameMap.get( actionName );
                if ( !( actionNameMap.containsKey( actionName ) ) )
                {
                    throw new ActionException( "Init action error, unrecognized '" + actionName + "'" );
                }
    ...
    public static void registerAction( String name, Class<?> actionClass )
    {
        ...
        {
            throw new RuntimeException( "Action named [" + name + "] already registered" );
        }
