...
public class FaultProcessor {
	...
	private FaultTransformer getTransformer(String sourceId, String targetId) {
		return ((FaultTransformer) this.transformers.get(new QName(targetId,
				sourceId)));
	}
	...
	public Exception processException(String sourceId, String targetId,
	...
	public Document processMessage(String sourceId, String targetId,
			Fault sourceFault) throws Exception {
		FaultTransformer transformer = getTransformer(sourceId, targetId);
		if (transformer == null) {
			throw new Exception("Transformer not found");
		}
		...
		XPath status_XPath = (XPath) sourceFault
				.getProperty("response.status.xpath");
		if (status_XPath != null) {
			status = status_XPath.valueOf(sourceDoc);
		}
		XPath statusText_XPath = (XPath) sourceFault
				.getProperty("response.statustext.xpath");
	...
	public void init(File installRoot) throws JDOMException, IOException {
		...
		for (File file : files) {
			...
			while (list.size() > 0) {
				...
				this.transformers.put(qname, transformer);
			}
		}
	}
}