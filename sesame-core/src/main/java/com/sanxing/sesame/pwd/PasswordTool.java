...
public class PasswordTool {
	...
	public static String decrypt(String passwd) {
		...
		try {
			...
			Document doc = new SAXBuilder().build(serverDir
					+ "/security/license.sesame");
			...
			FileInputStream fis1 = new FileInputStream(serverDir
					+ "/security/sesame-server.jks");
			KeyStore ks1 = KeyStore.getInstance("jks");
			PasswordCipher tool = PasswordCipher.getInstance();
			ks1.load(fis1, tool.decrypt(Base64.decode(storePasswd))
					.toCharArray());
			Key privateKey = ks1.getKey("sesame-server",
					tool.decrypt(Base64.decode(keyPasswd)).toCharArray());
