...
public class BaseServer
{
    ...
    private final List<ContainerInfo> containersInfos = new LinkedList();

    private final List<Container> runningContainers = new LinkedList();
    ...
    protected JMSProvider getJmsProvider()
    {
        if ( jmsProvider == null )
        {
            String providerClazz =
                SystemProperties.get( "com.sanxing.sesame.core.jms.provider",
                    "com.sanxing.sesame.core.jms.ActiveMQJMSProvider" );
            try
            {
                jmsProvider = ( (JMSProvider) Class.forName( providerClazz ).newInstance() );
            }
            catch ( Exception e )
            {
    ...
    protected DataSourceProvider getDataSourceProvider( int transactionManagerType )
    {
        ...
        {
            providerClazz =
                SystemProperties.get( "com.sanxing.sesame.core.jdbc.datasource.provider",
                    "com.sanxing.sesame.core.jdbc.DBCPProvider" );
        }
        try
        {
            datasourceProvider =
                ( (DataSourceProvider) Thread.currentThread().getContextClassLoader().loadClass( providerClazz ).newInstance() );
        }
        catch ( Exception e )
        {
            throw new RuntimeException( e.getMessage(), e );
    ...
    protected void registerMBeans()
    {
        try
        {
            ...
            MBeanHelper.registerMBean( getMBeanServer(), fileClient, MBeanHelper.getPlatformMBeanName( "file-client" ) );
    ...
    protected void startSystemService( ServerInfo serverInfo )
    {
        ...
        for ( int i = 0; i < classes.length; ++i )
        {
            ...
            try
            {
                ...
                Callback callback = (Callback) clazz.asSubclass( Callback.class ).newInstance();
        ...
        {
            Executor executor = executorFactory.createExecutor( id, callbacks );
            ObjectName name = MBeanHelper.getMBeanName( "ThreadPool", id );
            try
            {
                getMBeanServer().registerMBean( executor, name );
                LOG.debug( "Created thread pool .." + id + factoryImpl.getConfigs().get( id ) );
            }
            catch ( Exception e )
            {
    ...
    {
        for ( ContainerInfo ContainerInfo : myConfig.getContainerInfos() )
        {
            try
            {
        ...
        for ( ContainerInfo containerInfo : containersInfos )
        {
            try
            {
                LOG.info( "Starting container [" + containerInfo.getName() + "]" );
                ContainerContext context =
                    new ContainerContext( containerInfo.getName(), Platform.getEnv(), getNamingContext(),
                        getMBeanServer(), this );
                ...
                ClassLoader containerClassLoader = context.getContainerClassLoader();
                Thread.currentThread().setContextClassLoader( containerClassLoader );
                Class containerClazz = containerClassLoader.loadClass( containerInfo.getContainerClazz() );
