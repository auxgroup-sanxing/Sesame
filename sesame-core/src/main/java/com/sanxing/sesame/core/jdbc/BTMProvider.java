    ...
    implements DataSourceProvider
{
    ...
    public void provide( BaseServer server, DataSourceInfo dsInfo )
    {
        ...
        boolean useLRC = GetterUtil.getBoolean( configEle.getChildText( "use-lrc" ), false );
        String uniqueName = GetterUtil.getString( configEle.getChildText( "unique-name" ) );
        int maxPoolSize = GetterUtil.getInteger( configEle.getChildText( "max-pool-size" ), 10 );
        String driverClassName = GetterUtil.getString( configEle.getChildText( "driver-class" ) );
        String url = GetterUtil.getString( configEle.getChildText( "url" ) );
        String userName = GetterUtil.getString( configEle.getChildText( "username" ) );
        String password = GetterUtil.getString( configEle.getChildText( "password" ) );
        myDataSource = new PoolingDataSource();
        if ( useLRC )
        {
            myDataSource.setClassName( "bitronix.tm.resource.jdbc.lrc.LrcXADataSource" );
            myDataSource.getDriverProperties().setProperty( "driverClassName", driverClassName );
