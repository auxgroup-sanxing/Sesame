    ...
    extends BaseServer
{
    ...
    public JMXConnector getJMXConnectorByServer( String serverName )
    {
        try
        {
            if ( !( connectorCache.containsKey( serverName ) ) )
            {
                ...
                JMXConnector managedServerConnector =
                    JMXConnectorFactory.newJMXConnector( managedServerURL, environment );
                connectorCache.put( serverName, managedServerConnector );
            }
            JMXConnector connector = connectorCache.get( serverName );
            connector.connect();
            return connector;
        }
        catch ( IOException e )
    ...
    protected void registerMBeans()
    {
        ...
        MBeanHelper.registerMBean( getMBeanServer(), new FileServer(), MBeanHelper.getPlatformMBeanName( "file-server" ) );
