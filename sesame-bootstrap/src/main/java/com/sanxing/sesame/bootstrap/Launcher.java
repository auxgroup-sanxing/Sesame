...
public class Launcher
{
    ...
    /**
     * We have three properties file: default.properties -- embedded in this jar (not user editable) this is the place
     * to set java.io.tmp and debug options by users system.properties -- mandatory, will be picked up into context this
     * is place to set system.properties like workdir location etc (as today) sesame-test.properties -- optional, if
     * present, will override values from those above this is place to set test properties (like jetty port) etc We push
     * the whole app context into system properties, so that sesame[-test].properties can be used to set any system
     * properties (java.io.tmpdir, etc).
     */
    ...
    protected void canonicalizeEntry( final AppContext context, final String key )
        throws IOException
    {
    ...
    protected Properties loadProperties( final String resource, final boolean required )
        throws IOException
    {
        ...
        throws IOException
    {
        ...
        if ( commandMonitorPort != null )
        {
            new CommandMonitorThread( Integer.parseInt( commandMonitorPort ), new StopApplicationCommand(
                new Runnable()
                {
                    @Override
                    public void run()
        ...
        throws IOException
    {
        ...
        if ( port != null )
        {
            ...
            new KeepAliveThread( LOCALHOST, Integer.parseInt( port ), Integer.parseInt( pingInterval ),
                Integer.parseInt( timeout ) ).start();
        }
    }
