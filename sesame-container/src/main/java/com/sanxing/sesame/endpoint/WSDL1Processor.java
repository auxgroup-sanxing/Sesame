...
public class WSDL1Processor implements EndpointProcessor {
	...
	public void process(InternalEndpoint serviceEndpoint) {
		try {
			Document document = this.registry
			...
			if (!("http://schemas.xmlsoap.org/wsdl/".equals(document
					.getDocumentElement().getNamespaceURI()))) {
				if (LOG.isDebugEnabled()) {
					LOG.debug("Endpoint " + serviceEndpoint
							+ " has a non WSDL1 service description");
				}
				return;
			}
			WSDLReader reader = WSDLFactory.newInstance().newWSDLReader();
			reader.setFeature("javax.wsdl.verbose", false);
			Definition definition = reader.readWSDL(document.getBaseURI(),
					document);
			...
			if ((definition.getPortTypes().keySet().size() >= 1)
				...
				Service service = definition.getService(serviceEndpoint
						.getServiceName());
				if (service == null) {
					LOG.info("Endpoint "
							+ serviceEndpoint
				...
				if (port.getBinding() == null) {
					LOG.info("Endpoint "
							+ serviceEndpoint
							+ " has a service description, but no binding found");
					return;
				}
				if (port.getBinding().getPortType() == null) {
					LOG.info("Endpoint "
							+ serviceEndpoint
							+ " has a service description, but no port type found");
					return;
				}
				QName interfaceName = port.getBinding().getPortType()
						.getQName();
				if (LOG.isDebugEnabled()) {
					LOG.debug("Endpoint " + serviceEndpoint
							+ " implements interface " + interfaceName);
			...
			if (LOG.isDebugEnabled())
				LOG.debug(
						"Error retrieving interfaces from service description",
						e);
		}
	}
}