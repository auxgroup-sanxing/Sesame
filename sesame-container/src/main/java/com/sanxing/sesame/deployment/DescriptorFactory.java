...
public final class DescriptorFactory {
	...
	public static Descriptor buildDescriptor(final URL url) {
		try {
			...
			Schema schema = schemaFactory.newSchema(DescriptorFactory.class
					.getResource("/jbi-descriptor.xsd"));
			Validator validator = schema.newValidator();
			validator.setErrorHandler(new ErrorHandler() {
				public void warning(SAXParseException exception)
						throws SAXException {
					DescriptorFactory.LOG.debug("Validation warning on " + url
							+ ": " + exception);
				}

				public void error(SAXParseException exception)
						throws SAXException {
					DescriptorFactory.LOG.info("Validation error on " + url
							+ ": " + exception);
				}
			...
			validator.validate(new StreamSource(new ByteArrayInputStream(baos
					.toByteArray())));
			...
			Document doc = docBuilder.parse(new ByteArrayInputStream(baos
					.toByteArray()));
	...
	private static Services parseServiceUnit(Element child) {
		...
		services.setBindingComponent(Boolean.valueOf(
				getAttribute(child, "binding-component")).booleanValue());
		List provides = new ArrayList();
		List consumes = new ArrayList();
		for (Element e = W3CUtil.getFirstChildElement(child); e != null; e = W3CUtil
		...
		services.setProvides((Provides[]) provides
				.toArray(new Provides[provides.size()]));
		services.setConsumes((Consumes[]) consumes
				.toArray(new Consumes[consumes.size()]));
		return services;
	}
	...
	private static ServiceAssembly parseServiceAssembly(Element child) {
		...
		for (Element e = W3CUtil.getFirstChildElement(child); e != null; e = W3CUtil
			...
			if ("identification".equals(e.getLocalName())) {
				...
				for (Element e2 = W3CUtil.getFirstChildElement(e); e2 != null; e2 = W3CUtil
						.getNextSiblingElement(e2)) {
					if ("identification".equals(e2.getLocalName())) {
						...
						for (Element e3 = W3CUtil.getFirstChildElement(e2); e3 != null; e3 = W3CUtil
								.getNextSiblingElement(e3)) {
					...
					if ("connection".equals(e2.getLocalName())) {
						...
						for (Element e3 = W3CUtil.getFirstChildElement(e2); e3 != null; e3 = W3CUtil
							...
							if ("consumer".equals(e3.getLocalName())) {
								...
								consumer.setInterfaceName(readAttributeQName(
										e3, "interface-name"));
								consumer.setServiceName(readAttributeQName(e3,
										"service-name"));
								consumer.setEndpointName(getAttribute(e3,
										"endpoint-name"));
								cn.setConsumer(consumer);
							} else if ("provider".equals(e3.getLocalName())) {
								Provider provider = new Provider();
								provider.setServiceName(readAttributeQName(e3,
										"service-name"));
								provider.setEndpointName(getAttribute(e3,
										"endpoint-name"));
				...
				connections.setConnections((Connection[]) cns
						.toArray(new Connection[cns.size()]));
				serviceAssembly.setConnections(connections);
			}
		}
		serviceAssembly.setServiceUnits((ServiceUnit[]) sus
				.toArray(new ServiceUnit[sus.size()]));
	...
	private static Component parseComponent(Element child) {
		...
		component.setSharedLibraries((SharedLibraryList[]) sls
				.toArray(new SharedLibraryList[sls.size()]));
		if (ext != null) {
	...
	public static void checkDescriptor(Descriptor descriptor) {
		...
		if (descriptor.getVersion() != 1.0D) {
			violations
					.add("JBI descriptor version should be set to '1.0' but is "
		...
		if (descriptor.getComponent() != null)
			...
			violations
					.add("The jbi descriptor does not contain any informations");
		}
	...
	private static void checkComponent(List<String> violations,
			Component component) {
	...
	private static void checkServiceAssembly(List<String> violations,
	...
	private static void checkServiceUnit(List<String> violations,
			Services services) {
	}

	private static void checkSharedLibrary(List<String> violations,
