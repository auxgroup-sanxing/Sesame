...
public class AdapterComponent extends BindingComponent implements
		ServiceUnitManager {
	...
	private Map<String, Class<?>> transportImpls = new HashMap();

	private Map<String, List<Binding>> mappings = new ConcurrentHashMap();

	private Map<ServiceEndpoint, Binding> bindings = new ConcurrentHashMap();
	...
	public void onMessageExchange(MessageExchange exchange)
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Exchange [" + exchange.getExchangeId()
					+ "] arriving at BC [" + getContext().getComponentName()
					+ "]");
		...
		try {
			if (exchange.getRole() == MessageExchange.Role.PROVIDER) {
				...
				Binding binding = (Binding) this.bindings.get(endpoint);
				if (binding != null) {
					this.carrier.dispatchMessage(exchange, binding);
					return;
				}

				exchange.setStatus(ExchangeStatus.ERROR);
				exchange.setError(new BindingException("Component[" + getName()
						+ "], Binding not found"));
				send(exchange);
				return;
			}
			...
			LOG.error("Binding was not found, exchangeId:"
			...
			if (exchange.getRole() == MessageExchange.Role.PROVIDER) {
				exchange.setStatus(ExchangeStatus.ERROR);
				exchange.setError((e.getCause() != null) ? (Exception) e
						.getCause() : e);
				send(exchange);
			}
		}
		...
		try {
			...
			if (!(bindingXml.exists())) {
				throw new JBIException(
						"Can Not Found [binding.xml] in binding component: "
			...
			if (className != null) {
				this.bindingImpl = getClassLoader().loadClass(className)
						.asSubclass(DefaultBinding.class);
			}
			LOG.debug("Binding Implementor: " + this.bindingImpl);
			...
			if (allowableEl != null) {
				...
				for (Element el : list) {
					...
					if (attr != null) {
						...
						TransportFactory.register(el.attributeValue("name"),
								clazz.asSubclass(Transport.class));
						this.transportImpls.put(el.attributeValue("name"),
								clazz);
					}
				}
			}
			...
			if (codecEl != null) {
				...
				Decoder decoder = (Decoder) decoderClazz.asSubclass(
						Decoder.class).newInstance();
				decoder.init(getContext().getWorkspaceRoot());
				this.codec.setDecoder(decoder);
				...
				Encoder encoder = (Encoder) encoderClazz.asSubclass(
						Encoder.class).newInstance();
				encoder.init(getContext().getWorkspaceRoot());
				this.codec.setEncoder(encoder);

				className = codecEl.elementTextTrim("fault-handler");
				if ((className != null) && (className.length() > 0)) {
					Class handlerClazz = getClassLoader().loadClass(className);
					FaultHandler handler = (FaultHandler) handlerClazz
							.asSubclass(FaultHandler.class).newInstance();
					this.codec.setFaultHandler(handler);
				}
				...
				this.codec.setProperties(params);
				return;
			}
	...
	public void shutDown() throws JBIException {
		...
		Set<Map.Entry<String, Class<?>>> entrySet = this.transportImpls.entrySet();
		for (Map.Entry entry : entrySet) {
			Class clazz = (Class) entry.getValue();
			TransportFactory.unregister((String) entry.getKey(),
					clazz.asSubclass(Transport.class));
		}
		super.shutDown();
	}
	...
	protected void bind(ServiceUnit serviceUnit) throws BindingException,
		...
		try {
			...
			for (Iterator iter = ports.values().iterator(); iter.hasNext();) {
				...
				Constructor constructor = this.bindingImpl
						.getConstructor(new Class[0]);
				Binding bindingInst = (Binding) constructor
						.newInstance(new Object[0]);
				bindingInst.init(this.codec, serviceUnit, service, port);
				URI uri = bindingInst.bind();

				Transport transport = bindingInst.getTransport();
				String contextPath = (uri.getPath().length() > 0) ? uri
						.getPath() : "/";
				transport.addCarrier(contextPath, this.carrier);
				...
				this.bindings.put(endpoint, bindingInst);
			}
	...
	protected void unbind(ServiceUnit serviceUnit) throws BindingException,
			JBIException {
		Map ports = serviceUnit.getService().getPorts();
		for (Iterator iter = ports.values().iterator(); iter.hasNext();) {
			Port port = (Port) iter.next();
			ServiceEndpoint endpoint = getContext().getEndpoint(
					serviceUnit.getServiceName(), port.getName());
			...
			if (endpoint != null) {
				...
				if (binding != null) {
					...
					transport.removeCarrier(contextPath, this.carrier);
					binding.unbind();
					this.bindings.remove(endpoint);
	...
	private void addEntry(String contextPath, Binding binding) {
		LOG.debug("Add binding entry [" + binding + "] for context path: "
				+ contextPath);
		List list = (List) this.mappings.get(contextPath);
		if (list == null) {
			this.mappings.put(contextPath, list = new ArrayList());
		}
	...
	protected Binding[] getBindings(Transport transport, String path) {
		...
		List list = (List) this.mappings.get(key);
		if (list != null) {
