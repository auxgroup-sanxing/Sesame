...
public abstract class BaseTransport implements Transport {
	...
	private Map<String, List<Carrier>> carriers = new Hashtable();
	private URI uri;
		...
		if (config != null) {
			...
			try {
				...
				for (int len = nodes.getLength(); i < len; ++i) {
					...
					properties.put(prop.getNodeName(),
							W3CUtil.getElementText(prop));
				}
	...
	public void addCarrier(String contextPath, Carrier receiver) {
		List list = (List) this.carriers.get(contextPath);

		if (list == null) {
			this.carriers.put(contextPath, list = new ArrayList());
		}
	...
	protected void doPost(String contextPath, InputStream input,
			OutputStream output) throws MessagingException, BindingException {
	...
	protected void postMessage(MessageContext message)
		...
		try {
			...
			Exception ex = new RecognizeException(
					"Could not recognize action(tx-code)");
			...
			log.error(e.getMessage(), new ErrorRecord(message.getSerial()
					.longValue(), e));
			Binding binding = message.getBinding();
	...
	protected void sendFault(MessageContext message) throws MessagingException,
			BindingException {
		...
		OperationContext operation = binding.getOperationContext(message
				.getAction());
		if (operation == null) {
