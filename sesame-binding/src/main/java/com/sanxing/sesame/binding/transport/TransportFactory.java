...
public class TransportFactory {
	...
	public static Transport getTransport(Location location)
		...
		return getTransport(location.getURI(), location.getConfig(),
				location.getStyle());
	}
	...
	public static Transport getTransport(URI uri, Element config, String style)
		...
		TransportProxy proxy = (TransportProxy) transports.get(endpoint);
		if (proxy != null) {
		...
		Class clazz = null;
		if (scheme == null) {
			throw new BindingException("Invalid URL '" + uri
		...
		if ("local".equals(style)) {
			clazz = (Class) acceptors.get(scheme);
		} else if ("remote".equals(style)) {
			clazz = (Class) connectors.get(scheme);
		} else {
		...
		if (clazz == null) {
			throw new BindingException("Can not find transport for scheme '"
					+ scheme + "'");
		}
		try {
			Constructor constructor = clazz.getConstructor(new Class[0]);
			Transport transport = (Transport) constructor
	...
	public static String getSchemeDescription(String scheme) {
		Class acceptorClazz = (Class) acceptors.get(scheme);
		if (acceptorClazz != null) {
			Description description = (Description) acceptorClazz
					.getAnnotation(Description.class);
			return description.value();
		}
		Class connectorClazz = (Class) connectors.get(scheme);
		if (connectorClazz != null) {
			Description description = (Description) connectorClazz
					.getAnnotation(Description.class);
			return description.value();
		}
		return null;
	...
	public static void register(String scheme, Class<? extends Transport> clazz) {
		if (Acceptor.class.isAssignableFrom(clazz)) {
			if (acceptors.containsKey(scheme)) {
				throw new RuntimeException("Acceptor for Scheme [" + scheme
						+ "] already registered!");
			}
		...
		if (Connector.class.isAssignableFrom(clazz)) {
			if (connectors.containsKey(scheme)) {
				throw new RuntimeException("Connector for Scheme [" + scheme
						+ "] already registered!");
			}
