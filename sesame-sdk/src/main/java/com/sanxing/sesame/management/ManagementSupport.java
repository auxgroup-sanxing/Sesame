...
public final class ManagementSupport {
	...
	public static String createWarningMessage(String task, String info,
			List componentResults) {
		Message msg = new Message();
		msg.setTask(task);
		msg.setResult("SUCCESS");
	...
	public static String createFrameworkMessage(Message fmkMsg,
			List componentResults) {
		try {
			Document doc = createDocument();
			Element jbiTask = createChild(doc, "jbi-task");
			jbiTask.setAttribute("xmlns",
			...
			Element frmkTaskResult = createChild(jbiTaskResult,
					"frmwk-task-result");
			Element frmkTaskResultDetails = createChild(frmkTaskResult,
			...
			if (componentResults != null) {
				for (Iterator iter = componentResults.iterator(); iter
						.hasNext();) {
	...
	private static void appendTaskResultDetails(Element root, Message fmkMsg) {
		...
		if (fmkMsg.getMessage() != null) {
			Element taskStatusMessage = createChild(taskResultDetails,
					"task-status-msg");
		...
		if (fmkMsg.getException() != null) {
			Element exceptionInfo = createChild(taskResultDetails,
					"exception-info");
			createChild(exceptionInfo, "nesting-level", "1");
			createChild(exceptionInfo, "loc-token");
			createChild(exceptionInfo, "loc-message", fmkMsg.getException()
					.getMessage());
			...
			pw.close();
			stackTrace.appendChild(root.getOwnerDocument().createCDATASection(
					sw2.toString()));
		}
	}

	public static DeploymentException componentFailure(String task,
			String component, String info) {
		try {
	...
	public static Element createComponentMessage(Message msg) {
		...
		Element componentTaskResultDetails = createChild(componentTaskResult,
	...
	public static Element createComponentFailure(String task, String component,
	...
	public static Element createComponentWarning(String task, String component,
			String info, Exception e) {
		Message msg = new Message();
		msg.setTask(task);
		msg.setResult("SUCCESS");
