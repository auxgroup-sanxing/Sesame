...
public class ServiceUnit {
	...
	private Map<QName, ReferenceEntry> references = new HashMap();

	private Map<String, OperationContext> operationMap = new HashMap();
	...
	public ServiceUnit(File serviceUnitRoot) throws JBIException {
		...
		try {
			...
			this.definition = wsdlReader.readWSDL(locator);

			File jbiFile = new File(serviceUnitRoot, "META-INF/jbi.xml");
			DocumentBuilderFactory factory = DocumentBuilderFactory
					.newInstance();
			DocumentBuilder builder = factory.newDocumentBuilder();
			this.descriptor = builder.parse(jbiFile);

			loadSchemas(this.definition);
	...
	public String getExtentionAttribute(String attributeName) {
		NodeList nodes = this.descriptor.getElementsByTagName("services");
	...
	public NodeList getExtentionElements(String operationName) throws Exception {
		NodeList nodes = this.descriptor.getElementsByTagName("art:link");
		int i = 0;
	...
	private void loadSchemas(Definition wsdlDef) throws IOException {
		...
		if (!(list.isEmpty())) {
			...
			Element schemaEl = (extEl instanceof Schema) ? ((Schema) extEl)
					.getElement() : ((UnknownExtensibilityElement) extEl)
					.getElement();

			XmlSchema schema = this.schemaCollection.read(schemaEl,
			...
			while (it.hasNext()) {
				...
				if (item instanceof XmlSchemaInclude) {
					...
					this.schemaMap.put(opera, xsi.getSchema());
				}
			}
		}
	}
	...
	public XmlSchema getSchema(String operationName) {
		return ((XmlSchema) this.schemaMap.get(operationName));
	}

	public OperationContext getOperationContext(String operationName) {
		return ((OperationContext) this.operationMap.get(operationName));
	}
	...
	private void loadOperations() {
		NodeList items = this.descriptor.getElementsByTagName("art:link");
		String serviceName;
		for (int i = 0; i < items.getLength(); ++i) {
			...
			ref.setOperationName(QName.valueOf(W3CUtil.getChildText(el,
					"operation-name")));
			this.references.put(
					QName.valueOf(el.getAttribute("operation-name")), ref);
		}

		Set<Map.Entry> services = this.definition.getServices().entrySet();
		Map.Entry intfEntry;
		if (services.isEmpty()) {
			Operation operation;
			Set portTypes = this.definition.getPortTypes().entrySet();
			for (Iterator ite = portTypes.iterator(); ite.hasNext();) {
				...
				for (Iterator localIterator1 = operations.iterator(); localIterator1
						.hasNext();) {
					...
					this.operationMap.put(operation.getName(), context);
				}
			}
		} else {
			for (Map.Entry serviceEntry : services) {
				...
				for (Map.Entry portEntry : ports) {
					...
					for (BindingOperation operation : operations) {
						...
						ReferenceEntry ref = (ReferenceEntry) this.references
								.get(operationName);
						context.setReference(ref);
						context.setServiceUnit(this);
						context.setInterfaceName((portType == null) ? null
								: portType.getQName());
	...
	public ReferenceEntry getReference(QName operationName) {
		return ((ReferenceEntry) this.references.get(operationName));
	}

	public URL getResource(String name) throws IOException {
		File file = new File(this.serviceUnitRoot, name).getCanonicalFile();
	...
	public String getName() {
		return this.serviceUnitRoot.getName();
	}
}