...
public class TableEditOparation extends HttpServlet {
	...
	public String getValue(String kindid, String value, Connection conn) {
		...
		if (kindid.indexOf(":") >= 0) {
			...
			for (int i = 0; i < kindids.length; ++i) {
				if (!(kindids[i].substring(0, kindids[i].indexOf(":"))
						.equals(value)))
					continue;
				resultvalue = kindids[i].substring(kindids[i].indexOf(":") + 1,
						kindids[i].length());
	...
	public void insertLog(Connection con, String userid, String table_name,
			String field_name, String main_content, String before_modify,
			String after_modify) {
		Statement stmt = null;
		ResultSet rs = null;
		try {
			...
			String act_date = new SimpleDateFormat("yyyyMMdd")
					.format(new Date());

			String act_time = new SimpleDateFormat("HH:mm:ss")
					.format(new Date());
			...
			sql = "insert into oper_log (logno,userid,act_date,act_time,table_name,field_name,main_content,before_modify,after_modify) values ("
					+ count
					+ ",'"
					+ userid
					+ "','"
	...
	public String getTablelocalname(String tablename,
			HttpServletResponse response) throws IOException {
		try {
			File file = new File(getServletContext().getRealPath(
					"tablegroup/table.xml"));

			SAXBuilder builder = CommonUtil.newSAXBuilder();
			Document document = builder.build(file);
			Element root = document.getRootElement();
			Element userEl = (Element) XPath.selectSingleNode(root,
					"table[@id='" + tablename + "']");
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			this.useridname = ((String) user1.get("userid"));
			...
			if (action.equals("delete")) {
				...
				if (shouquan.equals("001")) {
					...
					String buserid = request.getParameter("buserid");
					String bpassword = request.getParameter("bpassword");
					if ((!(checkUser(cuserid, cpassword, "B")))
							|| (!(checkUser(buserid, bpassword, "A")))) {
						out.println("[shouquanerror]");
						return;
					}
				...
				for (int i = 0; i < indexs.length; ++i) {
					if (i == 0) {
						...
						if (indexs[i].substring(indexs[i].indexOf(":") + 1,
								indexs[i].lastIndexOf(":")).equals("string")) {
							wherecheck = indexs[i].substring(0,
									indexs[i].indexOf(":"))
									+ "='"
									+ request.getParameter(indexs[i].substring(
											0, indexs[i].indexOf(":"))) + "'";
						} else {
							wherecheck = indexs[i].substring(0,
									indexs[i].indexOf(":"))
						...
						column = column
								+ ","
								+ indexs[i]
										.substring(0, indexs[i].indexOf(":"));

						if (indexs[i].substring(indexs[i].indexOf(":") + 1,
								indexs[i].lastIndexOf(":")).equals("string")) {
							wherecheck = wherecheck
									+ " and "
									+ indexs[i].substring(0,
											indexs[i].indexOf(":"))
									+ "='"
									+ request.getParameter(indexs[i].substring(
											0, indexs[i].indexOf(":"))) + "'";
				...
				String sqlcheck = "select " + column + " from " + tablename
						+ " where " + where;
				...
				if (checkflag) {
					sqlcheck = "select count(*) from " + tablename + " where "
							+ wherecheck;
				...
				List list = getElement(getServletContext().getRealPath(
						"table/" + tablename + ".xml"));
				...
				for (Iterator iter = list.iterator(); iter.hasNext();) {
					...
					if (!(formitem.equals("checkbox"))) {
						...
						if ((canshuold != null)
								&& (!(canshuold.substring(
										canshuold.indexOf(":") + 1,
										canshuold.length()).equals(canshu)))) {
							updatebefore = updatebefore + canshuold;
							updateafter = updateafter
									+ canshuold.substring(0,
											canshuold.indexOf(":")) + ":"
									+ canshu;
						}
						...
						if (kindid.indexOf(":") < 0) {
							String sql1 = "select count(*) "
									+ kindid.substring(kindid.indexOf("from"),
											kindid.length());
				...
				for (int i = 0; i < where.length; ++i) {
					...
					String type = where[i].substring(where[i].indexOf(":") + 1,
							where[i].lastIndexOf(":"));
				...
				sql = "insert into " + tablename + " (" + sql1 + ") values ("
						+ sql2 + ")";
			...
			ex.printStackTrace();
			String message = new String(ex.getMessage().getBytes("UTF-8"),
					"ISO-8859-1");
	...
	public boolean checkUser(String userid, String password, String userlevel) {
		try {
			...
			if ((userEl != null)
					&& (password.equals(userEl.getAttributeValue("passwd")))) {
				if (userlevel.equals("C")) {
					return (!(userid.equals(this.useridname)));
				}

				if (userlevel.equals("B")) {
					return ((!(userid.equals(this.useridname))) && (!(userEl
							.getAttributeValue("userlevel").equals("C"))));
				}

				if (userlevel.equals("A")) {
					return ((!(userid.equals(this.useridname)))
							&& (!(userEl.getAttributeValue("userlevel")
									.equals("C"))) && (!(userEl
								.getAttributeValue("userlevel").equals("B"))));
				}
	...
	public String getcheckbox(String kindid, int k, HttpServletRequest request,
			String name) {
		String value = "";
		try {
			if (kindid.indexOf(":") < 0) {
