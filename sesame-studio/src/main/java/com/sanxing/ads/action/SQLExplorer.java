...
import com.sanxing.ads.ConnDispenser;
import com.sanxing.ads.utils.MessageException;
import com.sanxing.ads.utils.WebServletRequest;
import com.sanxing.ads.utils.WebUtil;
import java.io.File;
...
public class SQLExplorer extends HttpServlet implements Servlet {
	...
	private static final Logger LOG = Logger.getLogger(SQLExplorer.class);
		...
		try {
			...
			if (action != null)
				if (action.equals("getDBInfo")) {
					...
					JSONObject table = getTableMeta(getServletContext(),
							tableName);
					if ((table == null)
							|| (table.getJSONArray("columns").length() == 0)) {
						WebUtil.sendError(response,
								"没有取到表字段，请在数据库管理器中查看表是否存在");
						return;
					}
					out.print(table);
					...
					outputter.getFormat().setEncoding(
					...
					Element index = null;
					if (indexName != null) {
						index = (Element) XPath.selectSingleNode(root,
								"indices/index[@name='" + indexName + "']");
					}
					Element columns = (Element) XPath.selectSingleNode(root,
							"columns");
					document.detachRootElement();
					if (columns != null) {
						if (index != null) {
							...
							while (iter.hasNext()) {
								...
								if (column != null) {
									column.addContent(new Element("is_checked")
											.setText("true"));
									column.addContent(new Element("sort")
											.setText(indexCol
													.getAttributeValue("sort")));
								}
							}
						}
						columns.detach();
	...
	private void appendChild(String nodeId, HttpServletRequest request)
		...
		if (nodeId.equals("TABLE")) {
			...
			if (nodeId.startsWith("columns_"))
				return;
			System.out.println("appendChild->" + nodeId);
		}
	}

	private boolean removeNode(JSONObject node, String drop) throws Exception {
		...
		if ((meta != null) && (meta.equals("INDEX"))) {
			File file = new File(schemaFolder, "tables/"
					+ node.getString("table"));

			SAXBuilder builder = new SAXBuilder();
			Document document = builder.build(file);
			Element index = (Element) XPath.selectSingleNode(document,
					"/table/indices/index[@name='" + node.getString("name")
							+ "']");
	...
	private JSONObject loadTable(String table) throws Exception {
	...
	private void dropTable(String name) throws Exception {
		Connection conn = ConnDispenser.getConnection();
		try {
	...
	public String getChildren(String nodeId) throws Exception {
		...
		if (objectId.equals("ROOT")) {
			...
			for (int i = 0; i < list.length; ++i) {
				...
				treeNode.put("icon",
						"sqlexplorer_viewer/images/closedFolder.gif");
			...
			for (int i = 0; i < list.length; ++i) {
				...
				JSONObject treeNode = new JSONObject();
				treeNode.put("text",
						f.getName() + ":" + root.getAttributeValue("remarks"));
				...
				if ((list != null) && (list.length > 0))
					for (int i = 0; i < list.length; ++i) {
						...
						treeNode.put(
								"text",
								f.getName() + ":"
										+ root.getAttributeValue("remarks"));

						treeNode.put("cls", "folder");
						treeNode.put("id", "view_" + f.getName());
						treeNode.put("icon",
								"sqlexplorer_viewer/images/view.gif");
						treeNode.put("allowDelete", true);
						treeNode.put("allowModify", true);
						treeNode.put("allowSet", true);
						treeNode.put("allowBrowse", true);
						treeNode.put("name", f.getName());
						treeNode.put("remarks",
								root.getAttributeValue("remarks"));
						result.put(treeNode);
					}
			}
			...
			if (procedureFolder.exists()) {
				...
				if ((list != null) && (list.length > 0))
					for (int i = 0; i < list.length; ++i) {
						...
						treeNode.put("icon",
								"sqlexplorer_viewer/images/stored_procedure.gif");
						treeNode.put("allowDelete", true);
						treeNode.put("allowModify", true);
						treeNode.put("allowSet", true);
						treeNode.put("allowBrowse", true);
			...
			while (iter.hasNext()) {
				...
				if ((typeDscr.equals("char")) || (typeDscr.equals("varchar"))) {
					typeDscr = typeDscr + "("
							+ elem.getChildText("column_size") + ")";
				}
				treeNode.put("text", elem.getChildText("column_name") + " <b>"
						+ typeDscr + "</b>");
		...
		try {
			if (action != null)
				if (action.equals("getChildren")) {
					...
					boolean result = removeNode(node,
					...
					JSONArray connObjs = new JSONArray(
					...
					int tsCount = 0;
					if ((transactionManager.equalsIgnoreCase("stm"))
							&& (datas.length() > 0)) {
					...
					Connection conn = ConnDispenser.getConnection();
					try {
						if ((direction.equals("bidi"))
								|| (direction.equals("db"))) {
							meta2db(conn, folder, request);
						}
						if ((direction.equals("bidi"))
								|| (direction.equals("meta"))) {
					...
					try {
						...
						if ((null == tableName) || (tableName.length() < 1)) {
							...
							boolean result = singleReverse(conn, folder,
									tableName);
							out.print(result);
						}
					} finally {
	...
	private boolean saveConnections(JSONArray connObjs) throws Exception {
	...
	private boolean saveDataSources(JSONArray datas) throws Exception {
	...
	private JSONArray loadConnections() throws JDOMException, JSONException {
		JSONArray result = new JSONArray();
	...
	private JSONArray loadDataSources() throws JDOMException, JSONException {
		JSONArray result = new JSONArray();
	...
	private void throwTableNotFound() throws Exception {
		throw new MessageException("表不存在，可能已被删除");
	}

	public static JSONObject getMulTableMeta(ServletContext servletContext,
			String tableName) throws JDOMException, IOException, JSONException {
			...
			while (iter.hasNext()) {
				...
				if (type.indexOf("char") > -1) {
					...
					column.put("dateFormat", "Y-m-d");
						...
						|| (type.equals("money")) || (type.equals("numeric"))) {
					type = "float";
				...
				column.put("allowBlank",
						"true".equals(elem.getChildText("is_nullable")));
	...
	public static JSONObject getTableMeta(ServletContext servletContext,
			String tableName) throws JDOMException, IOException, JSONException {
		File schemaFolder = new File(servletContext.getRealPath("sqlex/schema"));
		File file = new File(schemaFolder, "tables/" + tableName);
		if (!(file.exists()))
	...
	public static JSONObject getViewMeta(ServletContext servletContext,
			String viewName) throws JDOMException, IOException, JSONException {
		File schemaFolder = new File(servletContext.getRealPath("sqlex/schema"));
	...
	private void meta2Table(File file, Connection conn) throws Exception {
		SAXBuilder builder = new SAXBuilder();
		DatabaseMetaData dbMeta = conn.getMetaData();
		Document document = builder.build(file);
		...
		if (columns != null) {
			...
			if (rs.next()) {
				...
				do {
					...
					Element column = (Element) XPath.selectSingleNode(columns,
		...
		for (Iterator index = indexList.iterator(); index.hasNext();) {
			...
			if (indexCols.size() > 0) {
				...
				String prefix = ((indexType != null) && (indexType
						.equalsIgnoreCase("unique"))) ? "UNIQUE" : "";

				String sqlText = "CREATE " + prefix + " INDEX "
						+ indexElem.getAttributeValue("name") + " ON "
				...
				for (Iterator colIter = indexCols.iterator(); colIter.hasNext();) {
					...
					colNames.add(column.getAttributeValue("name")
							+ ((sort != null) ? " " + sort : ""));
				}

				sqlText = sqlText
						+ colNames.toString().replaceFirst("^\\[", "(")
								.replaceFirst("\\]$", ")");

				sqlText = sqlText
						+ (((indexType != null) && (indexType
								.equalsIgnoreCase("primary"))) ? " WITH PRIMARY"
								: "");
				try {
					ConnDispenser.executeDDL(conn, sqlText);
				} catch (SQLException e) {
	...
	private boolean meta2db(Connection conn, File folder,
	...
	private String getColumnDefine(Element column) {
		...
		def = " DEFAULT "
				+ ((type.toLowerCase().indexOf("char") >= 0) ? "'" + def + "'"
						: def);
		...
		if (type.toLowerCase().indexOf("char") >= 0)
			...
			type = type + " (" + column.getChildText("column_size") + ","
					+ column.getChildText("decimal_digits") + ")";
		}
		String nullable = column.getChildText("is_nullable");
		return name + " " + type
				+ ((nullable.equals("false")) ? " NOT NULL" : "") + def;
	}

	private boolean db2meta(Connection conn, File folder,
			HttpServletRequest request) throws Exception {
		SAXBuilder builder = new SAXBuilder();
		DatabaseMetaData dbMeta = conn.getMetaData();
		...
		if ((tableFlag != null) && (tableFlag.equals("on"))) {
			...
			while (rs.next()) {
				...
				if (table == null) {
					table = new Element("table");
					String remarks = rs.getString("REMARKS");
					table.setAttribute("remarks", (remarks == null) ? ""
							: remarks);
		...
		if ((viewFlag != null) && (viewFlag.equals("on"))) {
			...
			while (rs.next()) {
				...
				if (view == null) {
					...
					String remarks = rs.getString("REMARKS");
					view.setAttribute("remarks", (remarks == null) ? ""
							: remarks);
		...
		if ((procFlag != null) && (procFlag.equals("on"))) {
			...
			while (rs.next()) {
				...
				if (proc == null) {
					...
					String remarks = rs.getString("REMARKS");
					proc.setAttribute("remarks", (remarks == null) ? ""
							: remarks);
	...
	private void reverseColumn(DatabaseMetaData meta, Element table, String name)
		...
		while (rs.next()) {
			...
			if (column == null) {
				...
				column.addContent(new Element("column_name")
						.setText(columnName));
				column.addContent(new Element("type_name").setText(rs
						.getString("TYPE_NAME").toLowerCase()));

				if (rs.getString("TYPE_NAME").toLowerCase().equals("int")) {
					column.addContent(new Element("column_size").setText("10"));
					column.addContent(new Element("decimal_digits")
							.setText("0"));
				} else {
					column.addContent(new Element("column_size").setText(rs
							.getString("COLUMN_SIZE")));
					column.addContent(new Element("decimal_digits").setText(rs
							.getString("DECIMAL_DIGITS")));
				}

				column.addContent(new Element("is_nullable").setText((nullable
						.equals("YES")) ? "true" : "false"));
				column.addContent(new Element("column_def")
						.setText((def == null) ? "" : def));
				column.addContent(new Element("restriction").setText(""));
				column.addContent(new Element("remarks")
						.setText((remark == null) ? "" : remark));
			}
		}
		rs.close();
	}

	private void reverseIndices(DatabaseMetaData meta, Element table,
		...
		if (index.getContentSize() > 0) {
			index.setAttribute("name", (indexName == null) ? tableName + "_PK"
					: indexName);
		...
		while (rs.next()) {
			...
			if (sort != null)
				indexCol.setAttribute("sort", (sort.equals("D")) ? "DESC"
						: "ASC");
			index.addContent(indexCol);
		}
		rs.close();
	...
	private boolean update(String nodeId, HttpServletRequest request)
		...
		if (nodeId.startsWith("table_")) {
			...
			if (!(id.equals(request.getParameter("name")))) {
				file.renameTo(new File(schemaFolder, "tables/"
		...
		if (nodeId.startsWith("view_")) {
			...
			writeSchemaFile(document, file);
			if (!(id.equals(request.getParameter("name")))) {
				file.renameTo(new File(schemaFolder, "views/"
						+ request.getParameter("name")));
			}
	...
	private void validateTable(HttpServletRequest request, File folder,
	...
	private void validateIndex(HttpServletRequest request, Document document,
			String oldId) throws Exception {
		...
		if ((oldId == null) || (!(indexName.equals(oldId)))) {
			Element table = (Element) XPath.selectSingleNode(document,
					"/table/indices/index[@name='" + indexName + "']");
	...
	public static String getViewSQL(String viewName,
			ServletContext servletContext) throws JDOMException, IOException {
		File schemaFolder = new File(servletContext.getRealPath("sqlex/schema"));
		SAXBuilder builder = new SAXBuilder();
		Document document = builder.build(new File(schemaFolder, "views/"
				+ viewName));
