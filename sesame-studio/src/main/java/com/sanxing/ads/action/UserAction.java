...
public class UserAction extends HttpServlet implements Servlet {
		...
		try {
			if (action != null) {
				if (action.equals("getAuth")) {
					...
					Element accredit = (Element) XPath.selectSingleNode(root,
							"user[@userid='" + userId + "']/accredit[@type='"
									+ type + "']");
					if (accredit != null) {
					...
					System.out.println(getServletName() + " GET::" + action);
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (action == null) {
				if ((currUser == null)
						|| (!(currUser.get("userlevel").equals("S")))) {
				...
				if (currPwd != null) {
					...
					bytes = cipher.doFinal(bytes);
					currPwd = new StringBuffer(new String(bytes).trim())
							.reverse().toString();
				}
				...
				if (userEl != null) {
					...
					for (int len = roleArray.length(); i < len; ++i) {
						...
						userEl.addContent(new Element("role").setAttribute(
								"id", roleId));
					}
					writeUserFile(document, file);
				}
				...
				Document document = builder.build(file);
				Element root = document.getRootElement();
				Element roleEl = (Element) XPath.selectSingleNode(root,
						"roles/role[@id='" + roleId + "']");
				if (roleEl != null) {
					roleEl.setAttribute("name",
	...
	private void putAccredit(Element roleEl, String accredit)
		...
		if (accreditObj.has("menu")) {
			...
			accreditEl = (Element) XPath.selectSingleNode(roleEl,
					"accredit[@type='menu']");
			if (accreditEl == null)
				roleEl.addContent(accreditEl = new Element("accredit")
						.setAttribute("type", "menu"));
			accreditEl.removeContent();
			for (keys = subObj.keys(); keys.hasNext();) {
				String key = (String) keys.next();
				accreditEl.addContent(new Element(key).setText(subObj
						.getString(key)));
			}
		}
	...
	private void validateUser(String userId, Element root) throws Exception {
		Element userEl = (Element) XPath.selectSingleNode(root,
				"user[@userid='" + userId + "']");
		if (userEl == null)
			return;
	...
	private void validateRole(String roleId, Element root) throws Exception {
		Element roleEl = (Element) XPath.selectSingleNode(root,
				"roles/role[@id='" + roleId + "']");
		if (roleEl == null)
			return;
	...
	private void writeUserFile(Document document, File file) throws IOException {
