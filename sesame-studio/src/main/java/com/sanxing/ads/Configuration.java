...
import com.sanxing.ads.utils.CommonUtil;
import java.io.File;
...
public class Configuration implements ServletContextListener {
	...
	private static Element getDefaultConnection() throws JDOMException {
		Element connEl = (Element) XPath.selectSingleNode(prefs,
				"/prefs/database/connection[@default='default']");
		if (connEl == null)
			throw new JDOMException("没有找到缺省数据库连接配置");
		return connEl;
	}

	public static boolean useDataSource() throws JDOMException {
		Element connEl = getDefaultConnection();
		return (connEl.getAttribute("datasource") != null);
	}

	public static String getDataSource() throws JDOMException {
		Element connEl = getDefaultConnection();
		String jndi = connEl.getAttributeValue("datasource");
		return jndi;
	}

	public static String getConnetionUrl() throws JDOMException {
		Element connEl = getDefaultConnection();
		String url = connEl.getAttributeValue("url");
		return url;
	}

	public static String getDriverClass() throws JDOMException {
		Element connEl = getDefaultConnection();
		String driverClass = connEl.getAttributeValue("driver");
		return driverClass;
	}

	public static String getConnectionUser() throws JDOMException {
		Element connEl = getDefaultConnection();
		return connEl.getAttributeValue("user");
	}

	public static String getConnectionPasswd() throws JDOMException {
		Element connEl = getDefaultConnection();
		return connEl.getAttributeValue("password");
	}

	public static List getConnections() throws JDOMException {
		return XPath.selectNodes(prefs, "/prefs/database/connection");
	}

	public static List getDataSources() throws JDOMException {
		return XPath.selectNodes(server, "/server/jdbc/datasource");
	}

	public static void setConnections(List connections) throws JDOMException,
			IOException {
		Element databaseEl = (Element) XPath.selectSingleNode(prefs,
				"/prefs/database");
	...
	public static void setTransactionManager(String value)
		...
		Element tsmEl = (Element) XPath.selectSingleNode(server,
	...
	public static void setDataSources(List dataSources) throws JDOMException,
			IOException {
		Element jdbcEl = (Element) XPath.selectSingleNode(server,
				"/server/jdbc");
		if (jdbcEl == null) {
	...
	public static Element getJMS() throws JDOMException {
		return ((Element) XPath
				.selectSingleNode(server, "/server/jms/app-info"));
	}

	public static void setJMS(JSONArray jmsValue) throws Exception {
	...
	public static List getClusters() throws JDOMException {
	...
	public static void publishDataSource(JSONArray servers, List dataSources)
		...
		for (int i = 0; i < servers.length(); ++i) {
			...
			Element serverEl = (Element) XPath
					.selectSingleNode(cluster,
							"/managed-servers/server[server-name='"
									+ serverName + "']");
				...
				if (serverEl != null) {
					for (itr = dataSources.iterator(); itr.hasNext();) {
						...
						if ((dsList != null) && (!(dsList.isEmpty()))) {
							for (iterator = dsList.iterator(); iterator
									.hasNext();) {
								Element datasourceEl = (Element) iterator
										.next();
								if (datasourceEl != null) {
									String jndiName = datasourceEl
											.getChildText("jndi-name");
	...
	public static void abolishDataSource(List<String> dataSources)
		...
		if ((dataSources != null) && (!(dataSources.isEmpty())))
			for (Iterator it = dataSources.iterator(); it.hasNext();) {
				...
				List dsList = XPath.selectNodes(cluster,
						"/managed-servers/server/jdbc/datasource");
	...
	public static void writePrefsFile() throws IOException {
		XMLOutputter outputter = new XMLOutputter();
		outputter.setFormat(Format.getPrettyFormat().setEncoding("GBK")
				.setIndent("  "));
		FileOutputStream outStream = new FileOutputStream(
				getPrefsFile(servletContext));
		outputter.output(prefs, outStream);
		outStream.close();
	}

	public static void writeServerFile() throws IOException {
		XMLOutputter outputter = new XMLOutputter();
		outputter.setFormat(Format.getPrettyFormat().setEncoding("GBK")
				.setIndent("  "));
		FileOutputStream outStream = new FileOutputStream(
				getServerFile(servletContext));
	...
	public static File getAddressBookFile() {
		String statenet_home = System.getProperty("STATENET_HOME");
		return new File(statenet_home, "conf/address-book.xml");
	}

	public static File getServerFile(ServletContext servletContext) {
		String statenet_home = System.getProperty("STATENET_HOME");
		if (statenet_home == null)
			statenet_home = servletContext.getInitParameter("ConfigPath");
		return new File(statenet_home + File.separator + "conf"
				+ File.separator + "admin.xml");
	}
	...
	public static Element getSCMPrefs() throws JDOMException {
		Element scmEl = (Element) XPath.selectSingleNode(prefs, "/prefs/scm");
		return ((Element) scmEl.clone());
	}

	public static void setSCMPrefs(Element prefsEl) throws JDOMException {
		Element scmEl = (Element) XPath.selectSingleNode(prefs, "/prefs/scm");
		if (scmEl != null) {
			scmEl.detach();
		}
	...
	public static void writeClusterFile() throws IOException {
		XMLOutputter outputter = new XMLOutputter();
		outputter.setFormat(Format.getPrettyFormat().setEncoding("GBK")
				.setIndent("  "));
		FileOutputStream outStream = new FileOutputStream(
				getClusterFile(servletContext));
