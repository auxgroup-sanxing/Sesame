...
public class Outputter {
	...
	public static void datasetToExcel(Element tableMeta, ResultSet rs,
		...
		for (int count = meta.getColumnCount(); i <= count; ++i) {
			...
			Element column = (tableMeta == null) ? null : (Element) XPath
					.selectSingleNode(tableMeta,
							"columns/column[lower-case(column_name)='"
									+ colName.toLowerCase() + "']");
			...
			if (column != null) {
				...
				JSONTokener json = new JSONTokener(
						column.getChildText("restriction"));
				...
				if (token instanceof JSONArray) {
					...
					for (int len = array.length(); j < len; ++j) {
						...
						enumObj.put(obj.getString("value"),
								obj.getString("text"));
					}
				}
			}
