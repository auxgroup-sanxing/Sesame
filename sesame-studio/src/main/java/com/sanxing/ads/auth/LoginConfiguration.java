...
public class LoginConfiguration extends Configuration {
	...
	public LoginConfiguration() {
		URL url = super.getClass().getClassLoader()
				.getResource("login-config.xml");
		if (url == null) {
			throw new RuntimeException(
					"Can't find <login-config.xml> in class-path");
		}
		SAXReader reader = new SAXReader();
		try {
			this.conf = reader.read(url);
		} catch (DocumentException e) {
			this.conf = DocumentHelper.createDocument();
			this.conf.add(DocumentHelper.createElement("config"));
			throw new RuntimeException(e.getMessage(), e);
		}
	}

	public AppConfigurationEntry[] getAppConfigurationEntry(String name) {
		Element el = this.conf.getRootElement().element(name);
		if (el != null) {
			...
			for (Iterator iter = modules.iterator(); iter.hasNext();) {
				...
				AppConfigurationEntry entry = new AppConfigurationEntry(
						moduleName, controlFlag, options);
				list.add(entry);
			}
			return ((AppConfigurationEntry[]) list
					.toArray(new AppConfigurationEntry[list.size()]));
		}
