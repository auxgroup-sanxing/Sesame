package com.sanxing.ads.team;

import com.sanxing.ads.Application;
import com.sanxing.ads.Configuration;
import com.sanxing.ads.team.svn.SVNAuthenticationManager;
import com.sanxing.ads.team.svn.SVNSynchronizer;
...
public class SCM {
	...
	public static synchronized ThreeWaySynchronizer getSynchronizer(File location, 
		...
		ThreeWaySynchronizer synchronizer = (ThreeWaySynchronizer) syncCache
				.get(location);
	...
	public static synchronized ThreeWaySynchronizer getSynchronizer(
			File location) {
		ThreeWaySynchronizer synchronizer = (ThreeWaySynchronizer) syncCache
				.get(location);
		if (synchronizer == null) {
	        ...
	        try {
	            ...
	            ISVNWCDb.WCDbInfo info = db.readInfo(location.getAbsoluteFile(), new ISVNWCDb.WCDbInfo.InfoField[] {
					ISVNWCDb.WCDbInfo.InfoField.reposRootUrl });
	            if (info != null) {
			...
			try {
				...
				Element repoEl = (Element) XPath.selectSingleNode(prefsEl,
			...
			synchronizer = new SVNSynchronizer(repositoryURL, username,
					password);
			syncCache.put(location, synchronizer);
		}
		return synchronizer;
	...
	public static boolean isVersioned(File location) {
        ...
        try {
            db.open(SVNWCDbOpenMode.ReadOnly, null, false, false);
            ISVNWCDb.WCDbInfo info = db.readInfo(location.getAbsoluteFile(), new ISVNWCDb.WCDbInfo.InfoField[] {
				ISVNWCDb.WCDbInfo.InfoField.revision });
            if (info != null) {
    			return true;
            }
        } catch(SVNException e) {
		...
		SVNWCAccess wcAccess = SVNWCAccess.newInstance(null);
		try {
			SVNAdminArea area = wcAccess.open(location.getParentFile(), false,
					false, false, 0, Level.FINEST);
	...
	public static Map<String, Object> getVersionInfo(File location) {
        ...
        try {
            db.open(SVNWCDbOpenMode.ReadOnly, null, false, false);
            ISVNWCDb.WCDbInfo info = db.readInfo(location.getAbsoluteFile(), new ISVNWCDb.WCDbInfo.InfoField[] {
				ISVNWCDb.WCDbInfo.InfoField.reposRelPath,
				ISVNWCDb.WCDbInfo.InfoField.reposRootUrl,
				ISVNWCDb.WCDbInfo.InfoField.changedAuthor,
				ISVNWCDb.WCDbInfo.InfoField.changedRev,
				ISVNWCDb.WCDbInfo.InfoField.lock,
				ISVNWCDb.WCDbInfo.InfoField.revision });
            if (info != null) {
    			Map result = new HashMap();
    			result.put("url", info.reposRootUrl.toString() + "/" + info.reposRelPath.toString().replace("\\", "/"));
		...
		try {
			...
			result.put("committed-rev",
					Long.valueOf(entry.getCommittedRevision()));
	...
	private static SVNRepository getSVNRepository(String repositoryURL)
			throws SVNException {
		SVNRepository repository = (SVNRepository) reposCache
				.get(repositoryURL);
		if (repository == null) {
	...
	public static PathEntry[] getEntries(String repositoryURL, String path)
		...
		try {
			...
			Collection<SVNDirEntry> entries = repository.getDir(path,
					SVNRevision.HEAD.getNumber(), null, c);
