        ...
        while ( bindingNames.hasNext() )
        {
            ...
            if ( ( bind != null ) && ( bind.getPortType() != null ) )
            {
                bind.getPortType().setQName(
                    new QName( source.getTargetNamespace(), bind.getPortType().getQName().getLocalPart() ) );

                if ( log.isDebugEnabled() )
                {
                    log.debug( "portype name changed to " + bind.getPortType().getQName() );
                }
                bind.getPortType().setUndefined( false );
                bind.getPortType().getOperations();
                for ( int i = 0; i < bind.getPortType().getOperations().size(); ++i )
                {
                    Operation operation = (Operation) bind.getPortType().getOperations().get( i );
                    if ( operation.getInput() != null )
                    {
                        QName inputMsgName =
                            new QName( source.getTargetNamespace(),
                                operation.getInput().getMessage().getQName().getLocalPart() );

                        operation.getInput().getMessage().setQName( inputMsgName );
                        if ( log.isDebugEnabled() )
                        {
                            log.debug( "input param's name changed to " + operation.getInput().getMessage().getQName() );
                        }
                    }
                    if ( operation.getOutput() != null )
                    {
                        ...
                        operation.getOutput().getMessage().setQName( outputMsgName );
                        if ( log.isDebugEnabled() )
                        {
                            log.debug( "out param's name changed to " + operation.getOutput().getMessage().getQName() );
                        }
                    }
                    Iterator faultIterator = operation.getFaults().keySet().iterator();
                    while ( faultIterator.hasNext() )
                    {
                        Object faultkey = faultIterator.next();
                        Fault fault = (Fault) operation.getFaults().get( faultkey );
                        Message faultMSG = fault.getMessage();
                        String faultLocalName = faultMSG.getQName().getLocalPart();
                        QName faultQname = new QName( source.getTargetNamespace(), faultLocalName );
                        faultMSG.setQName( faultQname );
                        if ( log.isDebugEnabled() )
                        {
                            log.debug( "fault param's name changed to " + faultMSG.getQName() );
                        }
                    }
                }
            }
        ...
        throws XmlSchemaSerializerException
    {
        if ( source.getImports().size() != 0 )
        {
            ...
            while ( iterImports.hasNext() )
            {
                ...
                Vector wsdlImports = (Vector) source.getImports().get( importQName );
                for ( int i = 0; i < wsdlImports.size(); ++i )
                {
                    ...
                    if ( source.getTypes() != null )
                    {
                        ...
                        for ( int j = 0; j < schemaSize; ++j )
                        {
                            source.getTypes().removeExtensibilityElement( (ExtensibilityElement) newSchema.get( 0 ) );
                        }

                        List mergedSchemas = mergedDef.getTypes().getExtensibilityElements();
                    ...
                    else
                    {
                        ...
                        if ( importTypes != null )
                        {
                            List schemas = mergedDef.getTypes().getExtensibilityElements();
