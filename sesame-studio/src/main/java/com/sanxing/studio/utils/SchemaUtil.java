...
public class SchemaUtil
{
            ...
            && ( ( appEl = annoEl.getChild( "appinfo", ns ) ) != null ) )
        {
            ...
            for ( iter = list.iterator(); iter.hasNext(); )
            {
                ...
                else
                {
                    try
                    {
                        component.put( child.getName(), Double.parseDouble( value ) );
        ...
        throws JSONException, JDOMException
    {
        ...
        else
        {
            ...
            if ( ( typeEl = element.getChild( "complexType", ns ) ) != null )
            {
                ...
                for ( it = children.iterator(); it.hasNext(); )
                {
                    ...
                    if ( childName.equals( "attribute" ) )
                    {
                        ...
                        XPath xpath = XPath.newInstance( "ns:simpleType/ns:restriction/ns:enumeration[not(@used)]" );
                        xpath.addNamespace( "ns", ns.getURI() );
                        Element enumEl = (Element) xpath.selectSingleNode( child );
                        if ( enumEl != null )
                        {
                    ...
                    {
                        if ( ( childName.equals( "sequence" ) ) || ( childName.equals( "all" ) ) )
                        {
                            List list = child.getChildren();
                            for ( iter = list.iterator(); iter.hasNext(); )
                            {
                                Element el = (Element) iter.next();
                                int minOccurs = Integer.parseInt( el.getAttributeValue( "minOccurs", "1" ) );
                                int i = 0;
                                do
                                {
                                    JSONObject comp = new JSONObject();
                                    comp.put( "tag", el.getName() );
                                    comp.put( "name", el.getAttributeValue( "name" ) );
                                    comp.put( "stateful", false );
                                    items.put( comp );
                                    generateUI( el, comp );
                                }
            ...
            else if ( ( typeEl = element.getChild( "simpleType", element.getNamespace() ) ) != null )
            {
                component.put( "name", element.getAttributeValue( "name" ) );
                component.put( "tag", element.getName() );
                String label = getDocumentation( element );
                ...
                if ( ( restrictionEl = typeEl.getChild( "restriction", ns ) ) != null )
                {
                    ...
                    else
                    {
                        ...
                        setFieldType( component, type );
                        Element patternEl = restrictionEl.getChild( "pattern", ns );
                        if ( patternEl != null )
                        {
                            component.put( "regex", patternEl.getAttributeValue( "value" ) );
                            component.put( "regexText", getDocumentation( patternEl ) );
                        }
                    }
                }
                else
    ...
    static void setFieldType( JSONObject field, String type )
        throws JSONException
    {
        if ( ( type.equals( "int" ) ) || ( type.equals( "integer" ) ) || ( type.equals( "short" ) )
            || ( type.equals( "long" ) ) )
        {
    ...
    public static Element schema2Xml( XmlSchema schema, String rootname, Element flowElement )
        throws Exception
    {
    ...
    static void generate( XmlSchemaType xsdType, XmlSchema schema, Element xmlRoot )
        throws Exception
    {
        ...
        while ( elements.hasNext() )
        {
            ...
            {
                Namespace childNS =
                    Namespace.getNamespace( element.getRefName().getPrefix(), element.getRefName().getNamespaceURI() );
                xmlRootElement.addNamespaceDeclaration( childNS );
                namespaceMap.put( childNS.getURI(), childNS.getPrefix() );
                if ( element.getQName() == null )
                {
                    ele =
                        new Element( elementName, element.getRefName().getPrefix(),
                            element.getRefName().getNamespaceURI() );
                }
                else
                {
                    String prifix = namespaceMap.get( element.getQName().getNamespaceURI() );
                    if ( prifix == null )
                    {
                        ele =
                            new Element( elementName, element.getRefName().getPrefix(),
                    ...
                    {
                        ele = new Element( elementName, prifix, element.getQName().getNamespaceURI() );
                    }
                }
            }
            else
            {
                if ( element.getQName() == null )
                {
                    ele = new Element( elementName, xmlRoot.getNamespacePrefix(), xmlRoot.getNamespaceURI() );
                }
                else
                {
                    String prifix = namespaceMap.get( element.getQName().getNamespaceURI() );
                    ...
                    {
                        ele =
                            new Element( elementName, xmlRoot.getNamespacePrefix(),
                                element.getQName().getNamespaceURI() );
                    }
                    else
                    {
            ...
            XmlSchemaType xsType =
                ( element.getRefName() != null ) ? schema.getElementByName( element.getRefName() ).getSchemaType()
    ...
    static Iterator<?> getElements( XmlSchemaType schemaType )
        throws Exception
    {
