...
public class WSDLParser
{
    ...
    private final Map<QName, XmlSchema> schemaMap;

    private final Map<String, Record> projectRecordMap;

    private final Map<String, String> serviceUnitMap;
    ...
    public WSDLParser()
    {
        ...
        projectRecordMap = new HashMap();

        serviceUnitMap = new HashMap();
    }
    ...
    public void loadSchemas( Definition wsdlDef )
        throws IOException
    {
        Map imports = wsdlDef.getImports();
        for ( Iterator iter = imports.entrySet().iterator(); iter.hasNext(); )
        ...
        for ( int i = 0; i < list.size(); ++i )
        {
            ...
            if ( extEl instanceof Schema )
            {
                ...
                XmlSchema schema = schemaCollection2.read( schemaEl, wsdlDef.getDocumentBaseURI() );
    ...
    public String getSchemaDescription( XmlSchemaAnnotated object )
    {
        ...
        if ( ann != null )
        {
            ...
            for ( int i = 0; i < collect.getCount(); ++i )
            {
                ...
                {
                    XmlSchemaDocumentation doc = (XmlSchemaDocumentation) collect.getItem( i );
    ...
    private void portTypeIndex( PortType type, Record base, RecordCollector recCollector )
    {
        ...
        for ( Operation opera : operations )
        {
            ...
            if ( inputEle != null )
            {
                ...
                for ( Part part : (Collection<Part>) parts.values() )
                {
                    ...
                    if ( schema != null )
                    {
                        XmlSchemaElement schemaEl = schema.getElementByName( part.getElementName() );

                        schemaIndex( schema, schemaEl.getSchemaType(), inrec, recCollector );
                    }
                    else
                    {
                        logger.debug( "No schema found for " + part.getElementName() );
            ...
            if ( outputEl != null )
            {
                ...
                for ( Part part : (Collection<Part>) parts.values() )
                {
                    ...
                    if ( schema != null )
                    {
                        ...
                        schemaIndex( schema, schemaEl.getSchemaType(), outrec, recCollector );
                    }
                    else
                    {
                        logger.debug( "No schema found for " + part.getElementName() );
    ...
    {
        if ( ( wsdl.getParentFile() != null ) && ( wsdl.getParentFile().getName() != null ) )
        {
    ...
    {
        File file = new File( wsdl.getParentFile().getParentFile().getParentFile(), "jbi.xml" );
    ...
    public Record getProjectRecord( File jbi )
    {
        ...
        try
        {
            ...
            if ( compNs != null )
            {
                String uri = serviceUnitMap.get( jbiPath );
                if ( uri == null )
                {
                    serviceUnitMap.put( jbiPath, compNs.getURI() );
                    logger.debug( "add map " + jbiPath );
                }
                return null;
            }

            Record record = projectRecordMap.get( jbiPath );
            if ( record != null )
            {
                return record;
            }
            ...
            if ( sa != null )
            {
                ...
                if ( identify != null )
                {
                    ...
                    org.jdom.Element desc = identify.getChild( "description", ns );
            ...
            record.addField( column );
            projectRecordMap.put( jbiPath, record );
    ...
    public Record getStdRecord( File wsdl )
    {
        ...
        Column column = new Column( "import wsdl location", getImportLocation( definition ), false );
        ...
        String component = serviceUnitMap.get( jbiFile.getAbsolutePath() );
    ...
    public void schemaIndex( XmlSchema schema, XmlSchemaType xsdType, Record base, RecordCollector recCollector )
    {
            ...
            while ( elements.hasNext() )
            {
                ...
                else
                {
                    ...
                    if ( xsType == null )
                    {
                        ...
                        {
                            xsType = refSchema.getElementByName( typeName ).getSchemaType();
                ...
                ElementRecord eleRec = new ElementRecord( element.getName(), description );
                ...
                if ( !( xsType instanceof XmlSchemaSimpleType ) )
                {
                    ...
                    {
                        schemaIndex( schema, element.getSchemaType(), base, recCollector );
                    }
