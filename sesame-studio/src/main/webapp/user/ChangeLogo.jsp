...
<body>
	...
	<form name="form" action="../UserAction" method="post">
		<input type="hidden" name="action" value="changeLogo"> <input
			type="hidden" name="userid" value="<%=userId%>"> <input
			type="hidden" name="userdsr" value="">
		<table style="width: 100%">
			<tr>
				<td align="left">选择图像:<br />
					<div
						style="height: 150px; background-color: window; border: 1px solid gray; overflow: auto; padding: 2px;">
						<%
        try
		{
	        ...
	        for(int i=0;i<files.length;i++)
			{
				...
				{
		%>
						<a href="javascript:select()"><img
							src="../images/logo/<%=file.getName()%>"
							title="<%=file.getName()%>" class="userLogo"
							onclick="select(this)"
							onmouseover="Ext.get(this).addClass('hiliteLogo');"
							onmouseout="if (this!=selectedLogo) Ext.get(this).removeClass('hiliteLogo');"></a>
						<%
				}
			}
		}
		...
		%>
			...
			<tr>
				<td align="right"><input type="button" onclick="submitCheck()"
					name="btnSubmit" value="更改图片" disabled="disabled"> <input
					type="button" value="取消" onclick="window.history.back(-1)">
				</td>
			</tr>
		</table>
	</form>
