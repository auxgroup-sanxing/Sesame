...
return {
	...
	init : function(){
		...
		// 流程脚本函数
		var scriptionPanel = new Ext.grid.EditorGridPanel({
			title: '脚本函数配置',
			columnLines: true,
			border: false, loadMask: true,
			store: new Ext.data.Store({
				...
				baseParams: {operation: 'loadScriptionPkg', component: component},
				reader: new Ext.data.JsonReader({
					successProperty: 'success',
					totalProperty : "count",
					idProperty: 'func-name',
				...
				autoSave: false,
	            sortInfo:{field: 'func-package', direction: "ASC"},
				listeners: {
					loadexception: function(proxy, obj, response){ _this.showException(response); }
				}
			}),
            columns: [
	            {
	            	header: "包名", sortable: true, dataIndex: 'func-package', 
	            	editor: new Ext.grid.GridEditor(new Ext.form.TextField({}))
	            }
	        ],
			viewConfig: {
			...
			'->',
			{
				...
				text: '保存',
				handler: function(button, e){
					scriptionPanel.stopEditing();
					_this.saveScriptions(scriptionPanel);
				}
			},{
				cls: 'x-btn-text-icon',
				icon: '../images/tool16/refresh.gif',
		...
		var viewport = new Ext.Viewport({
			...
			items: [
				new Ext.TabPanel({
					...
					deferredRender: true,
					items: [formPanel, funcPanel, convPanel, scriptionPanel, mapPanel]
				})
			]
		});
	...
	saveScriptions: function(scriptionGrid) {
		var _this = this;
		var store  = scriptionGrid.store;
		...
		scriptionGrid.bwrap.mask('正在保存脚本配置...', 'x-mask-loading');
		Ext.Ajax.request({
			method: 'POST', 
			url: 'component_ctrl.jsp',
			params: {
				operation: 'saveScriptionPkg',
				component: component,
				data: Ext.encode(array)
			},
			callback: function(options, success, response){
				scriptionGrid.bwrap.unmask();
				if (!success) {
					_this.showException(response);
				}
				else {
