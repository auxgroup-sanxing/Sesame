...
return {
		...
		var buttons = [
			{
				...
				listeners: {
					enable:function(button){
						if (isLocked != 'true' || isOriginalFlag != 'true')
							button.disable();
					}
				}
			},'-',{
				...
				cls: 'x-btn-text-icon',
				disabled: isLocked != 'true' || isOriginalFlag != 'true',
				listeners:{
					click:function(){
						_this.save();
					},
	...
	// 查看对应的源操作
	openUnit : function() {
		var title = decodeURIComponent(intfDesc) ? decodeURIComponent(intfDesc)+'['+ intfId + ']' : intfId;
		var viewer = '../explorer/intf_viewer.jsp?intf=';

		var currWin = top.Application.getDesktop().getActiveWindow();
		top.Application.openWindow(intf, title, viewer + intf + '&activeTab=1' + '&unitId=' + intfId);
		currWin.close();
	},
