...
String intf = schema.substring(0, schema.lastIndexOf('/'));
String intfId = request.getParameter("unitId");
String intfDesc = request.getParameter("unitDesc") == null ? "" : request.getParameter("unitDesc");
...
String isOriginalPublic = request.getParameter("isOriginalPublic");

try
{
	File xsdFile = Configuration.getWorkspaceFile(schema);
	File intfFolder = xsdFile.getParentFile();
	
	intfId = intfFolder.getName();
	
	JSONObject user = Authentication.getCurrentUser();
	String userName = user.optString("userid");
	
	ThreeWaySynchronizer sync = SCM.getSynchronizer(intfFolder.getParentFile().getParentFile());
	if(null == sync || (!sync.isVersioned(xsdFile))){
		isVersioned = "false";
	} else {
		File wsdlFile = new File(intfFolder, "public.wsdl");
...
<script type="text/javascript" src="message_viewer.js"></script>
</head>
<body>
<script type="text/javascript">
	...
	var targetNamespace = '';
	var isOriginalFlag = '<%=isOriginalPublic%>';
