...
//锁定操作
@SuppressWarnings("unchecked")
public String operationLock(HttpServletRequest request, HttpServletResponse response) throws Exception
{
	...
	File xmlFile = new File(unitFolder, xsdFile.getName().replaceAll("\\.xsd$",".xml"));
	...
	if(null == sync || (!sync.isVersioned(wsdlFile))){
		throw new Exception("该服务单元对应的WSDL文件：["+wsdlFile.getAbsolutePath()+"],不在版本库中!");
	}
	...
	if (props != null) {
		...
		if(null == lock){
			...
			if(null == lock){
				if (xmlFile.exists())	sync.update(xmlFile, 0);	// 更新对应的流程文件
				...
				if(userName.equals((String)props.get("lock.owner")))
					...
					throw new Exception("操作:["+xsdFile.getName().replaceAll("\\.xsd$","")+"],已经被用户:["+props.get("lock.owner")+"]锁定!");
			}
		}else{
			throw new Exception("服务单元对应的WSDL文件:["+wsdlFile.getAbsolutePath()+"], 已经被用户[:"+props.get("lock.owner")+"]锁定!");
		}
	}
...
//解锁操作
@SuppressWarnings("unchecked")
public String operationUnlock(HttpServletRequest request, HttpServletResponse response) throws Exception
{
	String schema = request.getParameter("schema");
...
public String getInputInterface(HttpServletRequest request, HttpServletResponse response) throws Exception
{
	...
	// 如果是代理服务单元,则服务单元目录应为对应的本地服务单元
	if (unit.contains("/server/")) {
		String xsdPart = schema.split("/")[schema.split("/").length - 1];
		unit = schema.replaceAll("/" + xsdPart, "");
	}
...
public String getStructure(HttpServletRequest request, HttpServletResponse response) throws Exception
{
    ...
    if (tag == null) {
		...
		XmlSchemaObjectTable elements = schema.getElements();
		//elements.getItem(new javax.xml.namespace.QName(schema.getTargetNamespace(), name));
