...
Ext.ux.CodePress = Ext.extend(Ext.form.Field, {
    ...
    , onRender : function(ct, position){
		...
		this.textarea.dom.style.display = 'none'; //hidden on render ["leo"]
	...
	, initialize : function() {
		...
		//force change event
		this.doc.addEventListener('keypress', function(evt) {
			if (evt.charCode!=0) {
				if (evt.ctrlKey) {
					switch (evt.charCode) {
					case 86: case 118:
					...
					case 90: case 122:
						//pressed Ctrl-Z
    					_this.fireEvent('change', _this, evt);
					}
				}
				else {
    				_this.fireEvent('change', _this, evt);
			...
			else if (evt.keyCode==8 || evt.keyCode==13) {
    			_this.fireEvent('change', _this, evt);
			}
		}, true);
