...
Ext.ux.UploadDialog.Dialog = function(config)
{
  var default_config = {
    ...
    Make_Reload: false,
    post_var_name: 'file',
    returnData : []	 // 用于接收文件上传成功后，服务端返回的信息
  }
...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
  ...
  fillToolbar : function()
  {
    ...
    tb.x_buttons.add = tb.add(new Ext.ux.UploadDialog.TBBrowseButton({
  ...
  updateRecordState : function(data)
  {
    if ('success' in data.response && data.response.success) {
      ...
      data.record.set('returnData',data.response.info)	//根据服务端返回的结构
    }
    else {
  ...
  onCloseButtonClick : function()
  {
    ...
    store.each(function(r){
      if (r.get('state') == Ext.ux.UploadDialog.FileRecord.STATE_FINISHED)
      	returnData.push(r.get('returnData'));
    });
  ...
  close: function(){
	this.setReturnData();
	Ext.ux.UploadDialog.Dialog.superclass.close.call(this);
  },
	
  hide : function(animateTarget, cb, scope){
	this.setReturnData();
	Ext.ux.UploadDialog.Dialog.superclass.hide.call(this,animateTarget, cb, scope);
  },
